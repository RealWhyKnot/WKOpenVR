...
# Recording retention prunes Logs\spacecal_log.*.txt by count/size; it never looks inside
...
$Candidates = @(Get-ChildItem -Path $LogDir -File -Filter "spacecal_log.*.txt")
if ($Name.Count -gt 0) {
	$Picked = @()
	...
	if ($Picked.Count -eq 0) {
		throw "No spacecal_log.*.txt recordings in $LogDir"
	}
}
