...
{
	return BuildRoot() / L"driver_wkopenvr" / L"resources" / L"facetracking" / L"host" / L"WKOpenVR.FaceModuleHost.exe";
}
...
std::filesystem::path MakeTempDir(const wchar_t* name)
{
	...
	dir /= std::wstring(L"WKOpenVR_E2E_") + std::to_wstring(GetCurrentProcessId()) + L"_" +
...
void CopyDirectoryContents(const std::filesystem::path& source, const std::filesystem::path& target)
{
...
public:
	...
	bool Start(const std::filesystem::path& exe, const std::vector<std::wstring>& args)
	{
		...
		BOOL ok = CreateProcessW(exe.wstring().c_str(), commandLine.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW,
...
ProcessResult RunProcess(const std::filesystem::path& exe, const std::vector<std::wstring>& args, DWORD timeoutMs)
{
	std::wstring commandLine = QuoteArg(exe.wstring());
	for (const auto& arg : args) {
		commandLine += L" ";
...
template <typename Predicate> bool WaitUntil(Predicate pred, std::chrono::milliseconds timeout)
{
...
public:
	...
	bool WaitForFloat(const std::string& address, float& out, std::chrono::milliseconds timeout)
	{
		return WaitUntil(
		    [&] {
	...
	bool WaitForString(const std::string& address, std::string& out, std::string& typetag,
...
private:
	...
	void Run()
	{
		while (running_) {
			...
			uint8_t buf[1024];
			int got = recvfrom(sock_, reinterpret_cast<char*>(buf), sizeof(buf), 0, nullptr, nullptr);
			if (got <= 0) continue;
	...
	std::vector<DecodedOsc> DecodeSnapshot()
	{
		...
		for (const auto& packet : packets) {
			oscrouter::OscDispatch(packet.data(), packet.size(),
			                       [&](const char* addr, const char* tag, const uint8_t* args, size_t argSize) {
				                       DecodedOsc msg;
...
std::string EncodeFaceHostMessage(const std::string& type, const std::string& uuid = {})
{
	std::string body;
...
bool SendFaceHostMessage(const std::wstring& pipeName, const std::string& wire)
{
	...
	HANDLE h = CreateFileW(pipeName.c_str(), GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (h == INVALID_HANDLE_VALUE) return false;

	DWORD written = 0;
	BOOL ok = WriteFile(h, wire.data(), static_cast<DWORD>(wire.size()), &written, nullptr);
	CloseHandle(h);
	return ok && written == wire.size();
}

bool ParseUnsignedAfter(const std::string& text, const std::string& prefix, uint16_t& out)
{
...
bool ParseFaceHostOscQueryPorts(const std::string& log, uint16_t& httpPort, uint16_t& oscPort)
{
	return ParseUnsignedAfter(log, "httpPort=", httpPort) && ParseUnsignedAfter(log, "oscPort=", oscPort);
}
...
bool SendOscStringLocalhost(uint16_t port, const std::string& address, const std::string& value)
{
...
void CreateFakeFaceModule(const std::filesystem::path& modulesDir, const std::string& uuid, const std::string& version)
{
...
void CreateLegacyVrcftCompatFaceModule(const std::filesystem::path& modulesDir, const std::string& uuid,
...
void CreateLoadableTestFaceModule(const std::filesystem::path& modulesDir, const std::string& uuid,
                                  const std::string& version)
{
	auto moduleDir = modulesDir / Utf8ToWide(uuid) / Utf8ToWide(version);
	WriteFileUtf8(moduleDir / L"manifest.json",
...
TEST(E2E, FaceHostFakeFramesReachFakeVrchat)
{
	...
	const std::string shmemName =
	    "WKOpenVR_E2E_Face_" + std::to_string(GetCurrentProcessId()) + "_" + std::to_string(GetTickCount64());
	...
	ProcessResult result = RunProcess(FaceHostPath(),
	                                  {
	                                      ...
	                                      L"--e2e-fake-frame-count",
	                                      L"4",
	                                      L"--e2e-fake-frame-interval-ms",
	                                      L"1",
	                                      L"--status-file",
	                                      statusPath.wstring(),
	                                      L"--log-file",
	...
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/JawOpen", jawLegacy, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/JawOpen", jawV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/JawOpen", jawCurrentV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/LeftEyeLid", lidLegacy, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/EyeOpenLeft", lidV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/EyeLidLeft", lidCurrentV2, 5000ms));
	...
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/MouthSmileLeft", smileLegacy, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/MouthSmileLeft", smileV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/MouthCornerPullLeft", cornerLegacy, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/MouthCornerPullLeft", cornerV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/v2/SmileFrownLeft", smileFrownCurrentV2, 5000ms));
	...
	WriteFileUtf8(allowListPath, "/avatar/parameters/FT/v2/JawOpen\n"
	...
	facetracking::FaceOscPublishCounts filteredCounts = facetracking::PublishFaceFrameOsc(processedFrame, &filter);
	...
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/FT/v2/JawOpen", jawJerryV2, 5000ms));
	ASSERT_TRUE(harness.receiver.WaitForFloat("/avatar/parameters/OSCm/Proxy/FT/v2/EyeLidLeft", lidJerryV2, 5000ms));
	ASSERT_TRUE(
	    harness.receiver.WaitForFloat("/avatar/parameters/Example/Nest/v2/SmileFrownLeft", smileJerryV2, 5000ms));
...
TEST(E2E, FaceHostLoadsTestModuleAndPublishesFrames)
{
	...
	const std::string shmemName =
	    "WKOpenVR_E2E_FaceLoadable_" + std::to_string(GetCurrentProcessId()) + "_" + std::to_string(GetTickCount64());
	protocol::FaceTrackingFrameShmem shmem;
	ASSERT_TRUE(shmem.Create(shmemName.c_str()));
	...
	ASSERT_TRUE(WaitUntil(
	    ...
	    30000ms))
	    << "host did not discover loadable test module. status: " << ReadFileUtf8(statusPath)
	    << " log: " << ReadFileUtf8(logPath);

	uint16_t httpPort = 0;
	uint16_t oscPort = 0;
	ASSERT_TRUE(
	    WaitUntil([&] { return ParseFaceHostOscQueryPorts(ReadFileUtf8(logPath), httpPort, oscPort); }, 30000ms))
	    << "host did not advertise OSCQuery ports. log: " << ReadFileUtf8(logPath);
	EXPECT_NE(oscPort, 9000u);

	const std::string hostInfo = HttpGetLocalhost(httpPort, "/?HOST_INFO");
	EXPECT_NE(hostInfo.find("\"OSC_PORT\": " + std::to_string(oscPort)), std::string::npos) << hostInfo;
	...
	EXPECT_NE(tree.find("\"FULL_PATH\":\"/avatar/change\""), std::string::npos) << tree;
	EXPECT_NE(tree.find("\"ACCESS\":2"), std::string::npos) << tree;
	EXPECT_EQ(tree.find("\"FULL_PATH\":\"/avatar/parameters/JawOpen\""), std::string::npos) << tree;

	ASSERT_TRUE(SendOscStringLocalhost(oscPort, "/avatar/change", "avatar-id"));
	ASSERT_TRUE(
	    WaitUntil([&] { return ReadFileUtf8(logPath).find("addr=/avatar/change") != std::string::npos; }, 5000ms))
	    << "host did not receive callback on advertised OSC port. log: " << ReadFileUtf8(logPath);
	...
	ASSERT_TRUE(WaitUntil(
	    [&] {
		    ...
		    return (frame.flags & 0x1u) != 0u && (frame.flags & 0x2u) != 0u && frame.eye_openness_l > 0.6f &&
		           frame.expressions[26] > 0.7f && frame.expressions[45] > 0.2f &&
		           frame.upstream_expressions[3] == 0.0f;
	    },
	    30000ms))
	    << "loadable test module did not publish expected frame. status: " << ReadFileUtf8(statusPath)
	    << " log: " << ReadFileUtf8(logPath);
...
TEST(E2E, FaceHostReloadsInstalledModulesAndDisablesSelection)
{
	...
	const std::string shmemName =
	    "WKOpenVR_E2E_FaceLifecycle_" + std::to_string(GetCurrentProcessId()) + "_" + std::to_string(GetTickCount64());
	protocol::FaceTrackingFrameShmem shmem;
	ASSERT_TRUE(shmem.Create(shmemName.c_str()));
	...
	ASSERT_TRUE(WaitUntil([&] { return std::filesystem::exists(statusPath); }, 30000ms))
	    << "face host did not write status. log: " << ReadFileUtf8(logPath);
	...
	ASSERT_TRUE(WaitUntil(
	    ...
	    8000ms))
	    << "host did not reload/select new module. status: " << ReadFileUtf8(statusPath)
	    << " log: " << ReadFileUtf8(logPath);

	ASSERT_TRUE(SendFaceHostMessage(pipeName, EncodeFaceHostMessage("SelectModule", "")));
	...
	ASSERT_TRUE(WaitUntil(
	    ...
	    8000ms))
	    << "host did not disable active module. status: " << ReadFileUtf8(statusPath)
	    << " log: " << ReadFileUtf8(logPath);
...
TEST(E2E, FaceHostResolvesLegacyBridgeManifestWithoutAdapterAssembly)
{
	...
	EXPECT_FALSE(std::filesystem::exists(modulesDir / Utf8ToWide(uuid) / L"1.0.0" / L"assemblies" /
	                                     L"OpenVRPair.FaceTracking.VrcftCompat.dll"));

	const std::string shmemName =
	    "WKOpenVR_E2E_FaceLegacy_" + std::to_string(GetCurrentProcessId()) + "_" + std::to_string(GetTickCount64());
	protocol::FaceTrackingFrameShmem shmem;
	ASSERT_TRUE(shmem.Create(shmemName.c_str()));
	...
	ASSERT_TRUE(WaitUntil(
	    [&] {
		    ...
		    return status.find("\"module_count\": 1") != std::string::npos &&
		           log.find("bridge.json -> upstream_assembly=VirtualDesktop.FaceTracking.dll") != std::string::npos;
	    },
	    30000ms))
	    << "host did not resolve legacy bridge manifest. status: " << ReadFileUtf8(statusPath)
	    << " log: " << ReadFileUtf8(logPath);
...
TEST(E2E, CaptionsFakeOutputReachesFakeVrchat)
{
	...
	std::string text;
	std::string typetag;
	ASSERT_TRUE(harness.receiver.WaitForString("/chatbox/input", text, typetag, 5000ms));
