...
function Assert-InstalledState {
    ...
    # Expected files derive from the shared payload manifest so this list
    # cannot drift from the installer inputs (the drift is what silently
    # broke nightly releases for a week in 2026-07/08). Entries without an
    # Installed mapping are staged behind install-time feature flags and are
    # not asserted here. Uninstall.exe is generated by the installer itself.
    $manifest = & (Join-Path $PSScriptRoot 'Test-InstallerInputs.ps1') -Manifest
    $expected = @((Join-Path $InstallDir 'Uninstall.exe'))
    foreach ($entry in $manifest) {
        if (-not $entry.Installed) { continue }
        if ($Payload -ne 'Current' -and $entry.Group -eq 'questapp') { continue }
        $root, $relative = $entry.Installed.Split(':', 2)
        $base = if ($root -eq 'driver') { $driverDir } else { $InstallDir }
        $expected += (Join-Path $base $relative)
    }
