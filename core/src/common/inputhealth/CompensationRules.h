...
inline float ApplyScalarCompensationValue(
	uint8_t kind,
	...
	float partner_value,
	bool has_partner,
	float partner_rest_offset = 0.0f,
	bool has_partner_rest_offset = false)
{
	const PathFamily family = ClassifyPathFamily(path);
	if (!AllowsDriverCompensation(family) ||
		!ScalarMetadataAllowsCompensation(kind, path, scalar_type, scalar_units))
	{
		return raw_value;
	}
	...
	if (IsTriggerRemapFamily(family)) {
		if (learned_trigger_min > 0.0f || learned_trigger_max > 0.0f) {
			const float max_value = learned_trigger_max > 0.0f
				? learned_trigger_max : 1.0f;
	...
	if (IsThumbstickAxisFamily(family)) {
		...
		if (learned_deadzone_radius > 0.0f) {
			...
			const float radius = has_partner
				? std::sqrt(value * value + radial_partner * radial_partner)
				: std::fabs(value);
