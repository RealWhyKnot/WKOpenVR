...
TEST(AutoRecoverySnapTest, FlagIsOneShot_NextCycleResumesBlend)
{
	...
	EXPECT_FALSE(ShouldBlendCycle(true, false, snapThisCycle)) << "Cycle N (recovery): must snap";
	snapThisCycle = false; // cleared at the end of cycle N
	EXPECT_TRUE(ShouldBlendCycle(true, false, snapThisCycle)) << "Cycle N+1: blending resumes";
}
...
static_assert(!ShouldBlendCycle(true, false, true), "snap flag must override continuous-mode blend");
static_assert(ShouldBlendCycle(true, false, false), "the only blend-true case is continuous + established + no snap");
static_assert(!ShouldBlendCycle(false, false, false), "non-continuous state must snap");
