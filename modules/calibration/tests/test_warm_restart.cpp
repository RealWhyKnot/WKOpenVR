...
TEST(WarmRestartTest, DoesNotEngageOnSubThresholdBlip)
{
	...
	EXPECT_FALSE(wr::ShouldEngage(in)) << "Brief proximity drops are sensor noise, not real warm restarts";
}
...
static_assert(wr::ShouldEngage(kHealthyWarmRestart), "Healthy input must evaluate to engage at compile time");
static_assert(!wr::ShouldEngage({false, false, 30.0, true, true}), "No rising edge means no engage, at compile time");
...
static_assert(!wr::ShouldEngage({false, true, 30.0, true, false}), "Wrong state means no engage, at compile time");
...
static_assert(wr::kMinAwaySeconds == 5.0, "kMinAwaySeconds changed -- update calibration_robustness memos");
static_assert(wr::kGraceSamples == 100, "kGraceSamples changed -- ~30 s of grace at 3.5 Hz; verify the new "
                                        "value with the latest tick-rate measurement");
static_assert(wr::kMaxAwaySeconds == 4.0 * 3600.0, "kMaxAwaySeconds changed -- update the cold-cal handoff rationale");
static_assert(wr::kPositionJumpFastPathM == 0.30, "kPositionJumpFastPathM changed -- update the dead-proximity-sensor "
                                                  "fallback rationale");
static_assert(wr::kColdStartGraceTicks == 100, "kColdStartGraceTicks changed -- ~30 s of session warmup at 3.5 Hz");
...
TEST(WarmRestartTest, PoseJumpFastPathEngagesBelowMinAway)
{
	...
	EXPECT_TRUE(wr::ShouldEngage(in)) << "Large HMD displacement bypasses the kMinAwaySeconds gate";
}
...
TEST(WarmRestartTest, MaxAwayCeilingSuppresses)
{
	...
	EXPECT_FALSE(wr::ShouldEngage(in)) << "Beyond kMaxAwaySeconds the snap is unsafe without re-validation";
}
...
TEST(WarmRestartTest, MaxAwayCeilingBoundaryIsInclusive)
{
	...
	EXPECT_TRUE(wr::ShouldEngage(in)) << "Exactly at the ceiling still engages (<= boundary)";
}
...
TEST(WarmRestartTest, PoseJumpBypassesMaxAwayCeiling)
{
	...
	EXPECT_TRUE(wr::ShouldEngage(in)) << "Pose-jump fast-path bypasses kMaxAwaySeconds";
}
...
TEST(WarmRestartTest, ColdStartSuppressesEngage)
{
	...
	EXPECT_FALSE(wr::ShouldEngage(in)) << "First few ticks of a session must not warm-restart";
}
...
TEST(WarmRestartValidationTest, BiasGateBlocksSettledEvenWithLowMad)
{
	...
	const wr::ValidationInputs lowDispersionLowBias{wr::kValidatedSettledMadM - 0.001, 25, false, 0.002};
	EXPECT_EQ(wr::EvaluateValidation(lowDispersionLowBias), wr::ValidationOutcome::Settled)
	    << "Low dispersion + low bias is the only configuration that "
	       "should Settle";

	const wr::ValidationInputs lowDispersionHighBias{wr::kValidatedSettledMadM - 0.001, 25, false,
	                                                 wr::kAcceptBiasTransM + 0.001};
	EXPECT_NE(wr::EvaluateValidation(lowDispersionHighBias), wr::ValidationOutcome::Settled)
...
TEST(WarmRestartValidationTest, BiasBoundaryAtAcceptThreshold)
{
	...
	const wr::ValidationInputs atCap{0.005, 25, false, wr::kAcceptBiasTransM};
	EXPECT_EQ(wr::EvaluateValidation(atCap), wr::ValidationOutcome::Settled);

	const wr::ValidationInputs justAbove{0.005, 25, false, wr::kAcceptBiasTransM + 0.0001};
	EXPECT_EQ(wr::EvaluateValidation(justAbove), wr::ValidationOutcome::Inconclusive);
}
...
static_assert(wr::kAcceptBiasTransM == 0.005, "kAcceptBiasTransM changed -- review the lever-arm rotation propagation "
                                              "rationale in RotationBiasPropagatesViaLeverArm");
static_assert(wr::kFailBiasTransM == 0.015, "kFailBiasTransM changed -- review the stale-profile fail rationale; "
