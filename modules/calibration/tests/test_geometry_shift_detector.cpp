...
TEST(CusumGeometryShiftTest, SmallSustainedResidualRiseDoesNotAccumulate)
{
	...
	for (int i = 0; i < 100; ++i) {
		EXPECT_FALSE(UpdateCusumGeometryShift(s,
		                                      /*current=*/9.6,
		                                      /*baseline=*/5.8));
		EXPECT_DOUBLE_EQ(s.S, 0.0);
...
TEST(CusumGeometryShiftTest, SustainedShiftFiresAfterSustainGate)
{
	...
	EXPECT_EQ(s.sustainedAboveThreshold, 0) << "Sustain counter must reset along with S";
}
...
TEST(CusumGeometryShiftTest, SpikeThenNormalResetsSustainCounter)
{
	...
	EXPECT_DOUBLE_EQ(s.S, 0.0);
	EXPECT_EQ(s.sustainedAboveThreshold, 0) << "Single tick at or below threshold must reset the sustain counter";
}
...
TEST(CusumGeometryShiftTest, OutSustainAtFireCapturesPreResetValue)
{
	...
	for (int i = 0; i < 3; ++i) {
		EXPECT_FALSE(UpdateCusumGeometryShift(s, 5.0, 0.0, spacecal::geometry_shift::kCusumDriftMm,
		                                      spacecal::geometry_shift::kCusumThreshold, &valueAtFire, &sustainAtFire));
		EXPECT_EQ(sustainAtFire, -1) << "no fire => out-param untouched (tick " << i << ")";
		EXPECT_DOUBLE_EQ(valueAtFire, -1.0);
	}
	EXPECT_TRUE(UpdateCusumGeometryShift(s, 5.0, 0.0, spacecal::geometry_shift::kCusumDriftMm,
	                                     spacecal::geometry_shift::kCusumThreshold, &valueAtFire, &sustainAtFire));
	EXPECT_GE(sustainAtFire, kMinSustainedSpikes) << "fire requires sustain >= kMinSustainedSpikes; out-param must "
	                                                 "expose that pre-reset value so the diagnostic log can confirm it";
	EXPECT_GT(valueAtFire, 0.0) << "value-at-fire out-param should mirror the pre-reset S";
	EXPECT_EQ(s.sustainedAboveThreshold, 0) << "post-fire, the state itself resets -- the out-param is the only "
...
TEST(CusumGeometryShiftTest, ResetClampPreventsNegativeAccumulation)
{
	...
	for (int i = 0; i < 1000; i++)
		UpdateCusumGeometryShift(s, 0.0, 0.0);
	EXPECT_DOUBLE_EQ(s.S, 0.0);
	EXPECT_EQ(s.sustainedAboveThreshold, 0);
