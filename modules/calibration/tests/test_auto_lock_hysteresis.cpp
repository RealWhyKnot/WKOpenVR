...
TEST(AutoLockHysteresisTest, RobustTranslDeviationRejectsOutlierSpike)
{
	...
	EXPECT_LT(mad, 0.003) << "MAD should ignore the two 25mm outliers and stay below the 3mm "
	                         "enter threshold. Got "
	                      << mad << " m.";
}
...
TEST(AutoLockHysteresisTest, RobustTranslDeviationDetectsSustainedDrift)
{
	...
	const double mad = al::RobustTranslDeviation(history);
	EXPECT_GT(mad, 0.005) << "Sustained drift across 15mm should produce MAD large enough to "
	                         "be meaningful. Got "
	                      << mad << " m.";
}
...
TEST(AutoLockHysteresisTest, RobustRotDeviationRejectsOutlierSpike)
{
	...
	for (int i = 0; i < 28; ++i) {
		...
		history.push_back(SampleRot(Eigen::Quaterniond(Eigen::AngleAxisd(angle, Eigen::Vector3d::UnitZ()))));
	}
	// Two outliers at 5 degrees.
	const double bigAngle = 5.0 * EIGEN_PI / 180.0;
	history.push_back(SampleRot(Eigen::Quaterniond(Eigen::AngleAxisd(bigAngle, Eigen::Vector3d::UnitZ()))));
	history.push_back(SampleRot(Eigen::Quaterniond(Eigen::AngleAxisd(bigAngle, Eigen::Vector3d::UnitZ()))));

	const double mad = al::RobustRotDeviation(history);
	const double enterThresh = 0.7 * EIGEN_PI / 180.0;
	EXPECT_LT(mad, enterThresh) << "Rotation MAD should ignore the two 5-deg outliers. Got " << mad << " rad ("
	                            << (mad * 180.0 / EIGEN_PI) << " deg).";
}
...
TEST(AutoLockHysteresisTest, BimodalNoiseStaysLockedThroughSpikes)
{
	...
	{
		const bool verdict =
		    al::VerdictWithHysteresis(al::RobustTranslDeviation(history), al::RobustRotDeviation(history), locked);
	...
	// rate.
	EXPECT_LE(flipCount, 1) << "Bimodal noise (10% spikes) should not flap the lock. flipCount=" << flipCount;
...
TEST(AutoLockHysteresisTest, BimodalNoiseEnterLockOnceAndHold)
{
	...
	EXPECT_EQ(unlockEvents, 0) << "Should never unlock under bimodal 10%-spike noise. unlockEvents=" << unlockEvents;
}
...
TEST(AutoLockPanicTest, TranslationBoundary)
{
	...
	EXPECT_TRUE(al::IsPanicLevelDeviation(0.040, 0.0)) << "40 mm sits exactly on the panic floor -- should fire";
