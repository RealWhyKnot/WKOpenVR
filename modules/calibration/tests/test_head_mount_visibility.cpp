...
TEST(HeadMountVisibility, HideTrackerMatchesPersistedSerialOutsideContinuous)
{
	...
	EXPECT_TRUE(hm::ShouldHideHeadMountTracker(ctx, 9u, "LHR-HEAD", "lighthouse"));
	EXPECT_TRUE(hm::ShouldQuashPublishedTrackerPose(ctx, 9u, "LHR-HEAD", "lighthouse"));
}
...
TEST(HeadMountVisibility, HideTrackerRejectsHmdSlot)
{
	...
	EXPECT_FALSE(hm::ShouldHideHeadMountTracker(ctx, vr::k_unTrackedDeviceIndex_Hmd, "LHR-HEAD", "lighthouse"));
}
...
TEST(HeadMountVisibility, HideTrackerRejectsDifferentSerialOrSystem)
{
	...
	EXPECT_FALSE(hm::ShouldHideHeadMountTracker(ctx, 9u, "LHR-WAIST", "lighthouse"));
	EXPECT_FALSE(hm::ShouldHideHeadMountTracker(ctx, 9u, "LHR-HEAD", "oculus"));
}
...
TEST(HeadMountVisibility, ContinuousTargetHideStillUsesTargetToggle)
{
	...
	EXPECT_TRUE(hm::ShouldQuashPublishedTrackerPose(ctx, 12u, "LHR-TARGET", "lighthouse"));
}
...
TEST(HeadMountVisibility, ContinuousTargetHideOnlyRunsInContinuousState)
{
	...
	EXPECT_FALSE(hm::ShouldQuashPublishedTrackerPose(ctx, 12u, "LHR-TARGET", "lighthouse"));
}
