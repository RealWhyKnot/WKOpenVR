...
void ShowVersionLine()
{
	...
	const float footerW = s_inUmbrella ? ImGui::GetContentRegionAvail().x : (ImGui::GetWindowWidth() - 20.0f);
	...
	if (driverConnected) {
		...
		ImGui::TextColored(pal.statusOk, "Driver: connected (v%u)", (unsigned)protocol::Version);
	}
	else if (!IsVRReady()) {
		DrawStatusDot(pal.dotPending);
		ImGui::TextColored(pal.statusPending, "Driver: waiting for SteamVR");
	}
	else {
		DrawStatusDot(pal.dotError);
		ImGui::TextColored(pal.statusError, "Driver: disconnected -- reinstall the SteamVR driver");
	}
...
void GetModeStatus(const char*& label, const char*& tooltip, ImVec4& accent)
{
	...
	if (!validProfile) {
		...
		const char* rejectReason = Metrics::lastRejectReason.empty() ? "" : Metrics::lastRejectReason.c_str();
