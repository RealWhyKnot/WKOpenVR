...
bool FloorCaptureSession::Observe(const Eigen::Affine3d& rawPose, int deviceId, const std::string& trackingSystem)
{
	...
	const size_t percentileIndex =
	    std::min(sorted.size() - 1, static_cast<size_t>(std::floor(static_cast<double>(sorted.size() - 1) * 0.15)));
	const size_t highPercentileIndex =
	    std::min(sorted.size() - 1, static_cast<size_t>(std::ceil(static_cast<double>(sorted.size() - 1) * 0.85)));
	...
	const double recentDriftMeters =
	    m_recentFloorSamples.size() >= 2 ? std::fabs(m_recentFloorSamples.back() - m_recentFloorSamples.front()) : 0.0;
	...
	const double smoothed = wasValid ? previousFloorY + (estimate - previousFloorY) * kSmoothing : estimate;
	...
	m_candidate.stable = nextCount >= 8 && jitterMeters <= 0.02 && recentDriftMeters <= kStableRecentDriftMeters;
	return floorMoved || markerMoved || deviceChanged;
}

std::vector<BoundaryVertex> BuildFloorMarkerVertices(const Eigen::Affine3d& pose, double floorY, double halfSizeMeters)
{
