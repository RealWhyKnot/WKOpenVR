...
double TargetFloorYForStandingFloor(const std::vector<BoundaryVertex>& targetSpace,
                                    const Eigen::AffineCompact3d& targetToStanding, double standingFloorY = 0.0);
...
bool ApplySteamVrFloorOffset(double measuredFloorYStanding, char* errorBuffer = nullptr, size_t errorBufferSize = 0);

bool ApplySteamVrFloorOffsetFromDevice(vr::TrackedDeviceIndex_t deviceId, char* errorBuffer = nullptr,
...
SteamVrFloorVerification EvaluateSteamVrFloorVerification(const StandingYSample& before, const StandingYSample& after,
                                                          double measuredFloorYStanding, double toleranceMeters = 0.05);
...
bool DeserializeChaperoneSnapshot(const std::vector<uint8_t>& bytes, ChaperoneSnapshot& snapshot,
