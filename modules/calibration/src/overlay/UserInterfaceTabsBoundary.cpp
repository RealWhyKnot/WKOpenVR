...
{
	return wkopenvr::boundary::BoundaryControllerMatchesTargetTrackingSystem(trackingSystem,
	                                                                         CalCtx.targetTrackingSystem);
}

Eigen::AffineCompact3d BoundaryTargetToStandingTransform()
{
	return wkopenvr::boundary::ProfileTransformFromCalibration(CalCtx.calibratedRotation, CalCtx.calibratedTranslation);
}

double BoundaryHeightToStanding(const std::vector<BoundaryVertex>& targetVertices, double targetY)
{
	if (CalCtx.boundary.standingSpace) {
		return targetY;
	}
	return wkopenvr::boundary::TransformHeightToStandingUniverse(targetVertices, targetY,
	                                                             BoundaryTargetToStandingTransform());
}

std::vector<BoundaryVertex> BoundaryVerticesToStanding(const std::vector<BoundaryVertex>& targetVertices,
                                                       const Eigen::AffineCompact3d& targetToStanding)
{
	return wkopenvr::boundary::TransformToStandingUniverse(targetVertices, targetToStanding);
}

const Eigen::AffineCompact3d& PreviewTargetToStandingTransform()
{
	if (s_capturePreviewTransformValid && s_capture.state() != wkopenvr::boundary::CaptureState::Idle) {
...
std::vector<BoundaryVertex> BoundaryVerticesToStanding(const std::vector<BoundaryVertex>& targetVertices)
{
	return BoundaryVerticesToStanding(targetVertices, BoundaryTargetToStandingTransform());
}

void TickBoundaryPreviewForTargetVerticesAtFloor(bool wantVisible, const std::vector<BoundaryVertex>& targetVertices,
                                                 double targetFloorY, bool closeLoop)
{
	...
	wkopenvr::boundary::TickBoundaryPreview(
	    true, BoundaryVerticesToStanding(targetVertices, targetToStanding),
	    wkopenvr::boundary::TransformHeightToStandingUniverse(targetVertices, targetFloorY, targetToStanding),
	    closeLoop, "target_vertices");
...
void TickBoundaryPreviewForStandingVerticesAtFloor(bool wantVisible,
                                                   const std::vector<BoundaryVertex>& standingVertices, double floorY,
                                                   bool closeLoop)
{
	...
	wkopenvr::boundary::TickBoundaryPreview(true, standingVertices, floorY, closeLoop, "standing_vertices");
}

void LogNativeBoundaryPreflight(const char* source, const wkopenvr::boundary::ChaperoneOutput& output,
                                const Eigen::AffineCompact3d& targetToStanding, bool standingSpace)
{
	const auto& d = output.diagnostics;
	const Eigen::Vector3d t = targetToStanding.translation();
	char lbuf[1024];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary_native_preflight] source=%s status=%d reason=%s ready=%d space=%s input_vertices=%zu "
	         "normalized_vertices=%zu area=%.3f floor=%.3f ceiling=%.3f origin_inside=%d origin_distance=%.3f "
	         "centroid=(%.3f,%.3f) bounds_x=(%.3f,%.3f) bounds_z=(%.3f,%.3f) play=(%.3f,%.3f) "
	         "transform_trans=(%.3f,%.3f,%.3f)",
	         source ? source : "unknown", static_cast<int>(output.status), output.reason ? output.reason : "",
	         output.ready() ? 1 : 0, standingSpace ? "standing" : "target", d.inputVertexCount, d.normalizedVertexCount,
	         d.areaMetersSq, d.floorY, d.ceilingY, d.originInsidePolygon ? 1 : 0, d.originDistanceMeters, d.centroidX,
	         d.centroidZ, d.bounds.xMin, d.bounds.xMax, d.bounds.zMin, d.bounds.zMax, d.playAreaX, d.playAreaZ, t.x(),
	         t.y(), t.z());
	Metrics::WriteLogAnnotation(lbuf);
	openvr_pair::common::DiagnosticLog("boundary_native_preflight", "%s", lbuf);
}

NativeBoundaryPushResult PushTargetBoundaryToChaperoneWithTransform(const Eigen::AffineCompact3d& targetToStanding,
                                                                    const char* source)
{
	...
	const double standingFloor = CalCtx.boundary.standingSpace
	                                 ...
	                                 : wkopenvr::boundary::TransformHeightToStandingUniverse(
	                                       CalCtx.boundary.vertices, CalCtx.boundary.floorY, targetToStanding);
	const double standingCeiling =
	    CalCtx.boundary.standingSpace
	        ? CalCtx.boundary.ceilingY
	        : wkopenvr::boundary::TransformHeightToStandingUniverse(
	              CalCtx.boundary.vertices, CalCtx.boundary.floorY + kAutoBoundaryWallHeightMeters, targetToStanding);
	const auto bounds = wkopenvr::boundary::ComputePolygonBoundsXZ(standingVertices);
	{
		const Eigen::Vector3d t = targetToStanding.translation();
		char lbuf[512];
		snprintf(lbuf, sizeof lbuf,
		         "[boundary] target push request: source=%s space=%s target_vertices=%zu standing_floor=%.3f "
		         "standing_ceiling=%.3f transform_trans=(%.3f,%.3f,%.3f) bounds_x=(%.3f,%.3f) bounds_z=(%.3f,%.3f)",
		         source ? source : "unknown", CalCtx.boundary.standingSpace ? "standing" : "target",
		         CalCtx.boundary.vertices.size(), standingFloor, standingCeiling, t.x(), t.y(), t.z(), bounds.xMin,
		         bounds.xMax, bounds.zMin, bounds.zMax);
		Metrics::WriteLogAnnotation(lbuf);
		openvr_pair::common::DiagnosticLog("boundary", "%s", lbuf);
	}

	const auto output = wkopenvr::boundary::BuildChaperoneOutput(standingVertices, standingFloor, standingCeiling);
	result.preflight = output;
	LogNativeBoundaryPreflight(source, output, targetToStanding, CalCtx.boundary.standingSpace);
	if (!output.ready()) {
		result.visualOnly = output.status == wkopenvr::boundary::ChaperoneOutputStatus::VisualOnlyNoStandingOrigin;
		result.message = result.visualOnly ? "Boundary can be previewed, but SteamVR native chaperone requires the "
		                                     "standing origin inside the polygon."
		                                   : "Boundary geometry is not valid for SteamVR native chaperone.";
		char obuf[224];
		snprintf(obuf, sizeof obuf, "[boundary] target push rejected before commit: status=%d reason=%s",
		...
		openvr_pair::common::DiagnosticLog(
		    ...
		    source ? source : "unknown", static_cast<int>(output.status), output.reason ? output.reason : "",
		    output.diagnostics.originInsidePolygon ? 1 : 0, output.diagnostics.originDistanceMeters,
		    result.visualOnly ? 1 : 0);
		return result;
	}

	result.attempted = true;
	const bool ok = wkopenvr::boundary::PushToChaperone(standingVertices, standingFloor, standingCeiling);
	if (!ok) {
		...
		openvr_pair::common::DiagnosticLog(
		    ...
		    source ? source : "unknown", static_cast<int>(output.status),
		    output.diagnostics.originInsidePolygon ? 1 : 0, output.diagnostics.originDistanceMeters);
	}
	else {
		...
		openvr_pair::common::DiagnosticLog(
		    ...
		    "source=%s vertices=%zu area=%.3f play=(%.3f,%.3f) floor=%.3f ceiling=%.3f", source ? source : "unknown",
		    output.diagnostics.normalizedVertexCount, output.diagnostics.areaMetersSq, output.diagnostics.playAreaX,
		    output.diagnostics.playAreaZ, output.diagnostics.floorY, output.diagnostics.ceilingY);
	}
	return result;
}
...
{
	const auto result = PushTargetBoundaryToChaperoneWithTransform(BoundaryTargetToStandingTransform(), "current");
	if (!result.applied) {
...
bool AdjustStandingZeroFloorY(double deltaY, vr::HmdMatrix34_t* committedOut = nullptr)
{
	...
	char lbuf[224];
	snprintf(lbuf, sizeof lbuf, "[boundary-floor] standing_zero shift dY=%.4f beforeY=%.4f afterY=%.4f committed=%d",
...
wkopenvr::boundary::BoundaryFloorSourceRequest BuildFloorSourceRequest(bool controllerContactValid = false,
                                                                       double controllerContactStandingY = 0.0)
{
	...
	request.controllerContactValid = controllerContactValid && std::isfinite(controllerContactStandingY);
	request.controllerContactStandingY = controllerContactStandingY;
	return request;
}

std::string FloorSourceRejectedSummary(const wkopenvr::boundary::BoundaryFloorSourceDecision& decision)
{
...
void PlayAreaRectYRange(const wkopenvr::boundary::SteamVrFloorSnapshot& snapshot, double& minY, double& maxY)
{
	minY = 0.0;
	maxY = 0.0;
...
void LogFloorSourceDecision(const char* context, const wkopenvr::boundary::SteamVrFloorSnapshot& snapshot,
                            const wkopenvr::boundary::BoundaryFloorSourceRequest& request,
                            const wkopenvr::boundary::BoundaryFloorSourceDecision& decision, int deviceId = -1,
                            const std::string& trackingSystem = {}, bool rawPoseValid = false)
{
	double rectMinY = 0.0;
	double rectMaxY = 0.0;
	...
	char lbuf[1400];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary-floor-source] context=%s valid=%d source='%s' boundary_floor=%.4f standing_floor=%.4f "
	         "rejects='%s' chaperone=%d state=%d play_size=%d play=(%.3f,%.3f) play_rect=%d rect_y=(%.3f,%.3f) "
	         "setup=%d standing_zero=%d standing_zero_t=(%.3f,%.3f,%.3f) target_transform=%d target_t=(%.3f,%.3f,%.3f) "
	         "saved_floor=%.4f controller_valid=%d controller_y=%.4f controller_device=%d controller_system='%s' "
	         "raw_pose=%d head_mode=%d head_device=%d head_target=%d driver_synth=%d",
	         context ? context : "unknown", decision.valid ? 1 : 0,
	         wkopenvr::boundary::BoundaryFloorSourceKindName(decision.source), decision.boundaryFloorY,
	         decision.standingFloorY, rejects.c_str(), snapshot.chaperoneAvailable ? 1 : 0,
	         static_cast<int>(snapshot.calibrationState), snapshot.playAreaSizeValid ? 1 : 0, snapshot.playAreaX,
	         snapshot.playAreaZ, snapshot.playAreaRectValid ? 1 : 0, rectMinY, rectMaxY,
	         snapshot.chaperoneSetupAvailable ? 1 : 0, snapshot.standingZeroValid ? 1 : 0,
	         snapshot.standingZeroToRaw.m[0][3], snapshot.standingZeroToRaw.m[1][3], snapshot.standingZeroToRaw.m[2][3],
	         request.targetTransformValid ? 1 : 0, transformT.x(), transformT.y(), transformT.z(),
	         request.savedBoundaryFloorY, request.controllerContactValid ? 1 : 0, request.controllerContactStandingY,
	         deviceId, trackingSystem.c_str(), rawPoseValid ? 1 : 0, static_cast<int>(CalCtx.headMount.mode),
	         CalCtx.headMount.deviceID, CalCtx.targetID, CalCtx.headMount.mode == HeadMountMode::DriverSynth ? 1 : 0);
	Metrics::WriteLogAnnotation(lbuf);
	openvr_pair::common::DiagnosticLog("boundary-floor-source", "%s", lbuf);
}

wkopenvr::boundary::BoundaryFloorSourceDecision
CurrentFloorSourceDecision(bool controllerContactValid = false, double controllerContactStandingY = 0.0,
                           bool logDecision = false, const char* logContext = "ui", bool ignoreSavedBoundary = false)
{
	const auto snapshot = wkopenvr::boundary::QuerySteamVrFloorSnapshot();
	auto request = BuildFloorSourceRequest(controllerContactValid, controllerContactStandingY);
	if (ignoreSavedBoundary) {
		request.hasSavedBoundaryFloor = false;
	}
	auto decision = wkopenvr::boundary::ResolveBoundaryFloorSource(snapshot, request);
	if (logDecision) {
		LogFloorSourceDecision(logContext ? logContext : (controllerContactValid ? "controller_sample" : "ui"),
		                       snapshot, request, decision);
	}
	return decision;
}

const wkopenvr::boundary::BoundaryFloorSourceDecision& CachedFloorSourceDecision(bool force = false)
{
	const double now = CalCtx.timeLastTick;
	if (force || !s_cachedFloorSourceInitialized || now >= s_nextFloorSourceRefreshAt) {
...
void TickBoundaryPreviewForTargetVertices(bool wantVisible, const std::vector<BoundaryVertex>& targetVertices,
                                          bool closeLoop)
{
	if (CalCtx.boundary.standingSpace) {
		TickBoundaryPreviewForStandingVerticesAtFloor(wantVisible, targetVertices, CalCtx.boundary.floorY, closeLoop);
		return;
	}
	TickBoundaryPreviewForTargetVerticesAtFloor(wantVisible, targetVertices, CalCtx.boundary.floorY, closeLoop);
}

void StartBoundaryCapture(const BoundaryControllerChoice* selectedController)
{
	const bool useTargetSpace = ShouldUseTargetSpaceForBoundaryController(selectedController);
	s_captureUsesStandingSpace = !useTargetSpace;
	s_capturePreviewTransform =
	    useTargetSpace ? BoundaryTargetToStandingTransform() : Eigen::AffineCompact3d::Identity();
	s_capturePreviewTransformValid = true;
	if (useTargetSpace) {
		s_captureFloorY = (!CalCtx.boundary.standingSpace && std::isfinite(CalCtx.boundary.floorY))
		                      ? CalCtx.boundary.floorY
		                      : wkopenvr::boundary::TargetFloorYForStandingFloor(CalCtx.boundary.vertices,
		                                                                         s_capturePreviewTransform, 0.0);
	}
	else {
	...
	s_capture.Start();
	s_spatialSession = wkopenvr::boundary::BoundaryCaptureSessionDescriptor(
	    useTargetSpace ? wkopenvr::boundary::TargetSpace(selectedController ? selectedController->trackingSystem
	                                                                        : CalCtx.targetTrackingSystem,
	                                                     s_capturePreviewTransform, s_capture.sessionId())
	                   : wkopenvr::boundary::StandingSpace(selectedController ? selectedController->trackingSystem
	                                                                          : std::string()),
	    selectedController ? selectedController->deviceId : -1,
	    selectedController ? selectedController->trackingSystem : std::string(), s_captureFloorY,
	    s_captureRequireTrigger, s_capture.sessionId());

	char lbuf[256];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary-capture] mode: space=%s floor_y=%.3f require_trigger=%d controller=%d system='%s' "
	         "target_match=%d transform_ready=%d",
	         s_captureUsesStandingSpace ? "standing" : "target", s_captureFloorY, s_captureRequireTrigger ? 1 : 0,
	         selectedController ? selectedController->deviceId : -1,
	         selectedController ? selectedController->trackingSystem.c_str() : "",
	         selectedController && selectedController->matchesTarget ? 1 : 0, BoundaryTransformReady() ? 1 : 0);
	Metrics::WriteLogAnnotation(lbuf);
...
std::string DeviceStringProperty(vr::IVRSystem* vrs, vr::TrackedDeviceIndex_t deviceId,
                                 vr::ETrackedDeviceProperty property)
{
	if (!vrs) return {};
	char buffer[vr::k_unMaxPropertyStringSize] = {};
	vr::ETrackedPropertyError err = vr::TrackedProp_Success;
	vrs->GetStringTrackedDeviceProperty(deviceId, property, buffer, vr::k_unMaxPropertyStringSize, &err);
...
std::vector<BoundaryControllerChoice> EnumerateBoundaryControllers(vr::IVRSystem* vrs)
{
	...
	vrs->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseStanding, 0.0f, standingPoses,
	...
	for (vr::TrackedDeviceIndex_t deviceId = 0; deviceId < vr::k_unMaxTrackedDeviceCount; ++deviceId) {
		...
		choice.role = static_cast<vr::ETrackedControllerRole>(
		    vrs->GetInt32TrackedDeviceProperty(deviceId, vr::Prop_ControllerRoleHint_Int32, &err));
		if (err != vr::TrackedProp_Success) {
			choice.role = vr::TrackedControllerRole_Invalid;
		}

		const auto& pose = standingPoses[deviceId];
		choice.poseValid = pose.bDeviceIsConnected && pose.bPoseIsValid &&
...
void ResolveBoundaryControllerSelection(const std::vector<BoundaryControllerChoice>& choices)
{
	auto choose = [&](bool targetOnly) -> int32_t {
		...
		for (const auto& choice : choices) {
			...
			selection.push_back({choice.deviceId, choice.role, choice.poseValid});
		}
		return wkopenvr::controller_input::ChoosePreferredController(selection.data(), selection.size(),
...
const BoundaryControllerChoice* SelectedBoundaryController(const std::vector<BoundaryControllerChoice>& choices)
{
	for (const auto& choice : choices) {
...
bool ShouldUseTargetSpaceForBoundaryController(const BoundaryControllerChoice* choice)
{
	return wkopenvr::boundary::BoundaryCaptureShouldUseTargetSpace(choice && choice->matchesTarget,
...
{
	char prefix[64];
	snprintf(prefix, sizeof prefix, "%s controller %d", ControllerRoleLabel(choice.role), choice.deviceId);
...
void TickBoundaryControllerIdentify()
{
	...
	vrs->TriggerHapticPulse(static_cast<vr::TrackedDeviceIndex_t>(s_identifyBoundaryControllerDeviceId), 0, 2400);
	s_nextIdentifyPulseAt = now + 0.075;
}

void DrawBoundaryControllerSelector(const std::vector<BoundaryControllerChoice>& choices)
{
	...
	if (ImGui::Combo("##boundary_controller", &current, labelPtrs.data(), static_cast<int>(labelPtrs.size()))) {
	...
	if (ImGui::IsItemHovered()) {
		ImGui::SetTooltip(
		    "Vibrates the selected controller so you can confirm which one will set the floor and draw the boundary.");
	}
}
...
std::vector<BoundaryVertex> BoundaryPreviewPathWithCursor(const std::vector<BoundaryVertex>& rawVertices,
                                                          const wkopenvr::boundary::FloorHitPreview* cursor)
{
	std::vector<BoundaryVertex> preview = rawVertices;
	if (cursor && cursor->valid) {
		if (preview.empty() || BoundaryDistanceSq(preview.back(), cursor->hit) > (0.01 * 0.01)) {
...
wkopenvr::boundary::SpatialPrimitive CapturePreviewPrimitive(wkopenvr::boundary::SpatialPrimitiveKind kind,
                                                             const std::vector<BoundaryVertex>& vertices,
                                                             bool closeLoop, wkopenvr::boundary::SpatialStyle style,
                                                             int layer, bool ageFade = false)
{
...
std::vector<wkopenvr::boundary::SpatialRenderCommand>
BuildCapturePreviewCommands(const std::vector<BoundaryVertex>& rawVertices,
                            const wkopenvr::boundary::FloorHitPreview* cursor)
{
	...
	if (closePreview && preview.size() >= 3) {
		...
		primitives.push_back(
		    CapturePreviewPrimitive(SpatialPrimitiveKind::PolygonFloorRegion, preview, true, fillStyle, 0));
	}
	...
	if (rawVertices.size() >= 1) {
		...
		primitives.push_back(CapturePreviewPrimitive(SpatialPrimitiveKind::PolylinePath, rawVertices, false, pathStyle,
		                                             2,
		                                             /*ageFade=*/true));
	}
	...
	if (cursor && cursor->valid) {
		...
		if (!rawVertices.empty()) {
			primitives.push_back(CapturePreviewPrimitive(SpatialPrimitiveKind::PolylinePath,
			                                             {rawVertices.back(), cursor->hit}, false, ghostStyle, 4));
		}
		if (closePreview && rawVertices.size() >= 3) {
			primitives.push_back(CapturePreviewPrimitive(SpatialPrimitiveKind::PolylinePath,
			                                             {cursor->hit, rawVertices.front()}, false, ghostStyle, 4));
		}
		...
		primitives.push_back(
		    CapturePreviewPrimitive(SpatialPrimitiveKind::Marker, {cursor->hit}, false, cursorStyle, 12));
	}

	const double standingFloorY =
	    (!s_captureUsesStandingSpace && BoundaryTransformReady())
	        ? TransformHeightToStandingUniverse(preview.empty() ? rawVertices : preview, s_captureFloorY,
	                                            PreviewTargetToStandingTransform())
	        : s_captureFloorY;
	auto pushStandingMarker = [&](double x, double z, SpatialStyle style, int layer) {
	...
	if (auto* vrs = vr::VRSystem()) {
		...
		vrs->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseStanding, 0.0f, poses, vr::k_unMaxTrackedDeviceCount);
		...
		if (hmdPose.bDeviceIsConnected && hmdPose.bPoseIsValid &&
			...
			hmdStyle.fill = false;
			pushStandingMarker(hmdPose.mDeviceToAbsoluteTracking.m[0][3], hmdPose.mDeviceToAbsoluteTracking.m[2][3],
			                   hmdStyle, 15);
		}
	}
...
void TickBoundaryPreviewForCapture(const wkopenvr::boundary::FloorHitPreview* cursor = nullptr)
{
	...
	wkopenvr::boundary::TickBoundaryPreview(true, commands, commands.front().floorY, "capture",
...
bool ApplyFloorFromStandingContactPose(const Eigen::Affine3d& standingContactPose, vr::TrackedDeviceIndex_t deviceId,
                                       const std::string& trackingSystem)
{
	...
	char lbuf[256];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary-floor] set from controller: device=%d system='%s' measured_standing_y=%.4f "
	         "floor_offset_prev=%.4f floor_offset_now=%.4f vertices=%zu",
	         static_cast<int>(deviceId), trackingSystem.c_str(), measuredStandingY, previousOffset,
	         CalCtx.floorOffsetMetersY, CalCtx.boundary.vertices.size());
	Metrics::WriteLogAnnotation(lbuf);
	openvr_pair::common::DiagnosticLog("boundary-floor", "%s", lbuf);
...
bool ApplyCapturedBoundary()
{
	...
	if (!std::isfinite(bounds.xMin) || !std::isfinite(bounds.xMax) || !std::isfinite(bounds.zMin) ||
	    !std::isfinite(bounds.zMax) || (bounds.xMax - bounds.xMin) < 0.35 || (bounds.zMax - bounds.zMin) < 0.35) {
	...
	const Eigen::AffineCompact3d applyTransform =
	    s_capturePreviewTransformValid ? s_capturePreviewTransform : BoundaryTargetToStandingTransform();
	{
		...
		char lbuf[384];
		snprintf(lbuf, sizeof lbuf,
		         "[boundary] apply captured: raw=%zu cleaned=%zu area=%.3f floor_y=%.3f space=%s "
		         "transform_trans=(%.3f,%.3f,%.3f)",
		         s_capture.rawVertexCount(), verts.size(), area, CalCtx.boundary.floorY,
		         CalCtx.boundary.standingSpace ? "standing" : "target", t.x(), t.y(), t.z());
		Metrics::WriteLogAnnotation(lbuf);
		openvr_pair::common::DiagnosticLog("boundary", "%s", lbuf);
	}
	SaveProfile(CalCtx);
	openvr_pair::common::DiagnosticLog(
	    ...
	    s_capture.rawVertexCount(), verts.size(), area, CalCtx.boundary.floorY,
	    CalCtx.boundary.standingSpace ? "standing" : "target", CalCtx.boundary.enabled ? 1 : 0);

	const auto nativeResult = PushTargetBoundaryToChaperoneWithTransform(applyTransform, "capture_apply");
	s_boundaryNativeStatus.clear();
	if (!nativeResult.applied) {
		s_boundaryNativeStatus = nativeResult.visualOnly ? "Saved locally; SteamVR native boundary skipped because the "
		                                                   "standing origin is outside the drawn area."
...
void DrawPolygonPreview(const std::vector<BoundaryVertex>& verts, bool closeLoop = true, bool activePath = false)
{
	if (verts.empty()) return;

	const bool authoringStanding = activePath ? s_captureUsesStandingSpace : CalCtx.boundary.standingSpace;
	const double authoringFloorY = activePath ? s_captureFloorY : CalCtx.boundary.floorY;
	const Eigen::AffineCompact3d targetToStanding =
	    activePath ? (s_capturePreviewTransformValid ? s_capturePreviewTransform : BoundaryTargetToStandingTransform())
	               : BoundaryTargetToStandingTransform();
	...
	auto standingMarkerToAuthoring = [&](double standingX, double standingZ, BoundaryVertex& out) {
		...
		const Eigen::Vector3d target = targetToStanding.inverse() * Eigen::Vector3d(standingX, 0.0, standingZ);
	...
	if (auto* vrs = vr::VRSystem()) {
		...
		if (hmdPose.bDeviceIsConnected && hmdPose.bPoseIsValid &&
		    hmdPose.eTrackingResult == vr::TrackingResult_Running_OK) {
			hmdMarkerValid = standingMarkerToAuthoring(hmdPose.mDeviceToAbsoluteTracking.m[0][3],
			                                           hmdPose.mDeviceToAbsoluteTracking.m[2][3], hmdMarker);
		}
	}
...
wkopenvr::boundary::ChaperoneOutput NativePreflightForAuthoringVertices(const std::vector<BoundaryVertex>& vertices,
                                                                        double authoringFloorY, bool standingSpace,
                                                                        const Eigen::AffineCompact3d& targetToStanding)
{
	if (vertices.size() < 3) {
		return {};
	...
	const auto standingVertices = standingSpace ? vertices : BoundaryVerticesToStanding(vertices, targetToStanding);
	const double standingFloor =
	    standingSpace
	        ? authoringFloorY
	        : wkopenvr::boundary::TransformHeightToStandingUniverse(vertices, authoringFloorY, targetToStanding);
	const double standingCeiling =
	    standingSpace ? authoringFloorY + kAutoBoundaryWallHeightMeters
	                  : wkopenvr::boundary::TransformHeightToStandingUniverse(
	                        vertices, authoringFloorY + kAutoBoundaryWallHeightMeters, targetToStanding);
	return wkopenvr::boundary::BuildChaperoneOutput(standingVertices, standingFloor, standingCeiling);
}

void DrawNativePreflightStatus(const std::vector<BoundaryVertex>& vertices, double authoringFloorY, bool standingSpace,
                               const Eigen::AffineCompact3d& targetToStanding)
{
	if (vertices.size() < 3) return;
	const auto output = NativePreflightForAuthoringVertices(vertices, authoringFloorY, standingSpace, targetToStanding);
	const auto& pal = openvr_pair::overlay::ui::GetPalette();
	if (output.ready()) {
		ImGui::TextColored(pal.statusOk, "SteamVR native eligible: origin inside, play area %.2f x %.2f m",
		                   output.diagnostics.playAreaX, output.diagnostics.playAreaZ);
	}
	else if (output.status == wkopenvr::boundary::ChaperoneOutputStatus::VisualOnlyNoStandingOrigin) {
		ImGui::TextColored(pal.statusWarn, "Local boundary OK; SteamVR native origin is %.2f m outside the drawn area.",
		                   output.diagnostics.originDistanceMeters);
	}
	else {
		ImGui::TextColored(pal.statusWarn, "SteamVR native pending: %s", output.reason ? output.reason : "not ready");
	}
}
...
void DrawBoundaryPreviewStatus()
{
	...
	const bool previewUnavailable =
	    status.uploadsDisabled || (status.created && status.uploadFailureCount > 0 && !status.visible);
	const ImVec4 color = previewUnavailable ? pal.statusWarn : (status.visible ? pal.statusOk : pal.statusIdle);
	if (previewUnavailable) {
		ImGui::TextColored(
		    color, "In-VR preview unavailable - draw still works; the floor line just won't show in the headset.");
	}
	else {
		ImGui::TextColored(color, "In-VR preview: %s", status.visible ? "showing in headset" : "idle");
	}
	if (status.created || status.uploadFailureCount > 0 || status.lastRasterHash != 0) {
		ImGui::TextDisabled("Preview detail: source=%s mode=%s vertices=%zu failures=%d last_error=%d/%s hash=%llu",
		                    status.lastSource, status.uploadMode, status.lastVertexCount, status.uploadFailureCount,
		                    status.lastError, status.lastErrorName,
		                    static_cast<unsigned long long>(status.lastRasterHash));
	}
	if (status.fileMarkersVisible || status.fileMarkerFailureCount > 0 || status.fileMarkerTextureReady) {
		ImGui::TextDisabled("Preview markers: %s count=%zu texture=%s failures=%d last_error=%d/%s",
		                    status.fileMarkersVisible ? "visible" : "hidden", status.fileMarkerCount,
		                    status.fileMarkerTextureReady ? "ready" : "missing", status.fileMarkerFailureCount,
		                    status.fileMarkerLastError, status.fileMarkerLastErrorName);
	}
}
...
{
	{
		...
		const BoundaryControllerChoice* selectedController = SelectedBoundaryController(controllerChoices);
		...
		ImGui::TextWrapped("Set the floor from the selected controller, then walk it around the play-space edge.");
		if (ImGui::IsItemHovered()) {
			ImGui::SetTooltip("Floor and boundary work in SteamVR standing space, independent of space calibration.\n"
		...
		if (state == wkopenvr::boundary::CaptureState::Idle) {
			...
			if (s_floorCapture.active()) {
				...
				ImGui::TextColored(
				    pal.statusWarn,
				    floor.valid ? "Floor preview is live. Hold still on the floor until it is stable, then apply."
				                : "Move the selected controller down to the floor.");
				if (floor.valid) {
					ImGui::TextDisabled("Measured floor %.2f m from device %d (%s)", floor.floorY, floor.deviceId,
					                    floor.trackingSystem.c_str());
					ImGui::SameLine();
					ImGui::TextDisabled("wall height auto");
					ImGui::TextDisabled("%zu samples, %.0f mm floor jitter, %s", floor.sampleCount,
					                    floor.jitterMeters * 1000.0,
					                    floor.stable ? "stable" : (floor.ready ? "ready" : "settling"));
					ImGui::TextDisabled(
					    "Recent drift %.0f mm. Apply lowers the rig so the controller rests on the headset floor.",
				...
				if (ImGui::Button("Apply floor")) {
					...
					const bool applied = ApplyFloorFromStandingContactPose(
					    floorPose, static_cast<vr::TrackedDeviceIndex_t>(floor.deviceId), floor.trackingSystem);
					if (applied) {
				...
				if (ImGui::IsItemHovered()) {
					ImGui::SetTooltip("Rest the controller on the floor and apply. Moves SteamVR's floor "
					                  "(standing-zero) so the controller sits at floor level.");
				}
				ImGui::SameLine();
			...
			ImGui::TextDisabled("Floor adjustment %.0f mm. Boundary wall height is automatic.",
			...
			if (floorSourceDecision.valid) {
				ImGui::TextDisabled("Detected floor: %s, boundary %.2f m, SteamVR %.2f m",
				                    ...
				                    floorSourceDecision.boundaryFloorY, floorSourceDecision.standingFloorY);
			}
			else {
			...
			if (!hasVerts) {
				...
				if (ImGui::IsItemHovered()) {
					ImGui::SetTooltip("After this starts, move the lighthouse controller around the floor edge. Apply "
					                  "or cancel when done.");
				}
			}
			else {
				...
				const double area = wkopenvr::boundary::AbsoluteAreaXZ(pts);
				ImGui::TextDisabled("%d pts, %.2f m^2", (int)CalCtx.boundary.vertices.size(), area);
				ImGui::SameLine();
				...
				DrawNativePreflightStatus(CalCtx.boundary.vertices, CalCtx.boundary.floorY,
				                          CalCtx.boundary.standingSpace, BoundaryTargetToStandingTransform());
			}
		}
			...
			if (s_captureRequireTrigger) {
				ImGui::TextDisabled("Trigger %s%s", s_captureLastTriggerHeld ? "held" : "released",
			...
			DrawNativePreflightStatus(BoundaryPreviewPathWithCursor(s_capture.vertices(), statusCursorPtr),
			                          s_captureFloorY, s_captureUsesStandingSpace,
			                          s_capturePreviewTransformValid ? s_capturePreviewTransform
			                                                         : BoundaryTargetToStandingTransform());
			DrawBoundaryPreviewStatus();
			ImGui::Spacing();
...
void DrawQuestAppPointerSection(ImVec2 panelSize)
{
	...
	openvr_pair::overlay::ui::DrawTextWrapped("Quest Boundary and headset app setup now live in the Quest App module.");
	ImGui::Spacing();
	ImGui::TextDisabled("Enable Quest App from the Modules tab for Developer Mode setup, "
...
std::string TrackingSystemName(vr::IVRSystem* vrs, vr::TrackedDeviceIndex_t deviceId)
{
	char buffer[vr::k_unMaxPropertyStringSize] = {};
	vr::ETrackedPropertyError err = vr::TrackedProp_Success;
	vrs->GetStringTrackedDeviceProperty(deviceId, vr::Prop_TrackingSystemName_String, buffer,
...
{
	Eigen::Quaterniond wfd(dp.qWorldFromDriverRotation.w, dp.qWorldFromDriverRotation.x, dp.qWorldFromDriverRotation.y,
	                       dp.qWorldFromDriverRotation.z);
	Eigen::Vector3d wfd_t(dp.vecWorldFromDriverTranslation[0], dp.vecWorldFromDriverTranslation[1],
...
void WriteBoundaryInputSummary(const BoundaryInputStats& stats, uint64_t sessionId, size_t rawCount)
{
	char lbuf[760];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary-capture] waiting: session=%llu raw=%zu target='%s' capture_space=%s controllers=%d matching=%d "
	         "skipped_system=%d state_failed=%d pose_ok=%d pose_invalid=%d last_device=%d last_system='%s' "
	         "chosen_device=%d chosen_system='%s' chosen_y=%.3f chosen_raw_pose=%d",
	         static_cast<unsigned long long>(sessionId), rawCount, CalCtx.targetTrackingSystem.c_str(),
	         s_captureUsesStandingSpace ? "standing" : "target", stats.trackedControllers, stats.matchingControllers,
	         stats.skippedTrackingSystem, stats.controllerStateFailed, stats.poseOk, stats.poseInvalid,
	         stats.lastDeviceId, stats.lastTrackingSystem.c_str(), stats.chosenDeviceId,
	         stats.chosenTrackingSystem.c_str(), stats.chosenY, stats.chosenRawPoseValid ? 1 : 0);
	Metrics::WriteLogAnnotation(lbuf);
	openvr_pair::common::DiagnosticLog("boundary-capture", "%s", lbuf);
}
...
void LogPersistentBoundaryStatusOnChange(size_t vertexCount)
{
	...
	static int s_sig = 0;
	const int sig = (status.visible ? 1 : 0) | (status.textureReady ? 2 : 0) | (status.uploadsDisabled ? 4 : 0) |
	                (status.plane.valid ? 8 : 0) | (static_cast<int>(vertexCount & 0xFFFFu) << 8);
	...
	char lbuf[256];
	snprintf(lbuf, sizeof lbuf,
	         "[boundary] persistent overlay: vertices=%zu plane_valid=%d visible=%d texture_ready=%d "
	         "uploads_disabled=%d file_markers=%zu",
	         vertexCount, status.plane.valid ? 1 : 0, status.visible ? 1 : 0, status.textureReady ? 1 : 0,
	         status.uploadsDisabled ? 1 : 0, status.fileMarkerCount);
	Metrics::WriteLogAnnotation(lbuf);
	openvr_pair::common::DiagnosticLog("boundary", "%s", lbuf);
}
...
void TickPersistentBoundaryOverlay()
{
	...
	// an upload failure -- status.visible is false until a clean upload).
	{
		...
		for (const auto& v : standingVerts) {
			mix(v.x);
			mix(v.y);
			mix(v.z);
		}
	...
	const auto commands = wkopenvr::boundary::BuildPersistentBoundaryCommands(standingVerts, standingFloor);
	wkopenvr::boundary::TickBoundaryPreview(true, commands, standingFloor, "persistent", /*showFileMarkers=*/false);
...
void CCal_TickBoundaryCapture()
{
	...
	for (vr::TrackedDeviceIndex_t deviceId = 0; deviceId < vr::k_unMaxTrackedDeviceCount; ++deviceId) {
		...
		if (!standingTrackedPose.bDeviceIsConnected || !standingTrackedPose.bPoseIsValid ||
		...
		candidate.standingPose = HmdMatrix34ToAffine(standingTrackedPose.mDeviceToAbsoluteTracking);
	...
	const auto chosenIt = std::find_if(candidates.begin(), candidates.end(), [](const ControllerCandidate& c) {
	...
	if (s_floorCapture.active()) {
		const std::string controllerType = DeviceStringProperty(vrs, chosen.deviceId, vr::Prop_ControllerType_String);
		const double controllerOriginY = chosen.standingPose.translation().y();
		const double contactOffset =
		    wkopenvr::boundary::ControllerFloorContactOffsetMeters(controllerType, chosen.standingPose);
		Eigen::Affine3d contactPose = chosen.standingPose;
		contactPose.translation().y() = wkopenvr::boundary::AdjustControllerFloorYForContact(
		    controllerOriginY, controllerType, chosen.standingPose);
		const bool updated =
		    s_floorCapture.Observe(contactPose, static_cast<int>(chosen.deviceId), chosen.trackingSystem);
		const auto& floor = s_floorCapture.candidate();
		if (floor.valid) {
			if (updated || !s_floorPreviewVisible) {
				const auto marker = wkopenvr::boundary::BuildFloorMarkerVertices(floor.pose, floor.floorY);
				TickBoundaryPreviewForStandingVerticesAtFloor(true, marker, floor.floorY, true);
			...
			if (updated) {
				...
				snprintf(
				    fbuf, sizeof fbuf,
				    "[boundary-floor] preview: samples=%zu ready=%d stable=%d jitter_mm=%.1f device=%d system='%s' "
				    "type='%s' origin_y=%.3f contact_offset=%.4f floor_y=%.3f controller=(%.3f,%.3f,%.3f)",
				    floor.sampleCount, floor.ready ? 1 : 0, floor.stable ? 1 : 0, floor.jitterMeters * 1000.0,
				    floor.deviceId, floor.trackingSystem.c_str(), controllerType.c_str(), controllerOriginY,
				    contactOffset, floor.floorY, floor.pose.translation().x(), floor.pose.translation().y(),
	...
	if (!s_captureUsesStandingSpace && !chosen.rawPoseValid) {
		...
		if (s_waitTicks == 1 || (s_waitTicks % 120) == 0) {
			...
			snprintf(wbuf, sizeof wbuf,
			         ...
			         static_cast<unsigned long long>(sessionId), static_cast<int>(chosen.deviceId),
	...
	const Eigen::Affine3d capturePose = s_captureUsesStandingSpace ? chosen.standingPose : chosen.rawPose;
	...
	if (s_captureRequireTrigger) {
		...
		s_captureLastTriggerReadable = vrs->GetControllerState(chosen.deviceId, &state, sizeof(state));
		if (s_captureLastTriggerReadable) {
			s_captureLastTriggerHeld = wkopenvr::controller_input::IsTriggerHeld(vrs, chosen.deviceId, state, 0.25f,
			                                                                     &s_captureLastTriggerReading);
		}
	}
	...
	const BoundaryVertex measuredCursor{capturePose.translation().x(), captureFloorY, capturePose.translation().z()};
	if (!s_smoothedPreviewCursorValid || s_smoothedPreviewCursorSession != sessionId) {
	...
	const bool accepted = s_capture.TickProjectedPosition(capturePose, s_captureLastTriggerHeld, captureFloorY);
	if (accepted) {
		s_waitTicks = 0;
		char cbuf[760];
		snprintf(
		    cbuf, sizeof cbuf,
		    "[boundary-capture] accepted controller_xz: session=%llu device=%d system='%s' raw=%zu floor_y=%.3f "
		    "space=%s trigger=%d raw_pos=(%.3f,%.3f,%.3f) standing_pos=(%.3f,%.3f,%.3f) capture_pos=(%.3f,%.3f,%.3f)",
		    static_cast<unsigned long long>(sessionId), static_cast<int>(chosen.deviceId),
		    chosen.trackingSystem.c_str(), s_capture.rawVertexCount(), captureFloorY,
		    s_captureUsesStandingSpace ? "standing" : "target", s_captureLastTriggerHeld ? 1 : 0,
		    chosen.rawPose.translation().x(), chosen.rawPose.translation().y(), chosen.rawPose.translation().z(),
		    chosen.standingPose.translation().x(), chosen.standingPose.translation().y(),
		    chosen.standingPose.translation().z(), capturePose.translation().x(), capturePose.translation().y(),
		...
		if (s_waitTicks == 1 || (s_waitTicks % 120) == 0) {
			char wbuf[384];
			snprintf(wbuf, sizeof wbuf,
			         "[boundary-capture] waiting: trigger session=%llu raw=%zu readable=%d held=%d button=%d axis=%d "
			         "value=%.3f trigger_axes=%d axis_prop_errors=%d",
			         static_cast<unsigned long long>(sessionId), s_capture.rawVertexCount(),
			         s_captureLastTriggerReadable ? 1 : 0, s_captureLastTriggerHeld ? 1 : 0,
			         s_captureLastTriggerReading.buttonPressed ? 1 : 0, s_captureLastTriggerReading.analogAxis,
			         static_cast<double>(s_captureLastTriggerReading.analogValue),
			         s_captureLastTriggerReading.triggerAxisCount, s_captureLastTriggerReading.propertyErrors);
			Metrics::WriteLogAnnotation(wbuf);
			openvr_pair::common::DiagnosticLog("boundary-capture", "%s", wbuf);
		}
	}
