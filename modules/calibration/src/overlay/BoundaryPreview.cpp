...
std::string ResolveFileMarkerTexturePath()
{
	...
	for (const char* name : kCandidates) {
		const std::filesystem::path source = std::filesystem::current_path() / "core" / "src" / "overlay" / name;
...
void DrawSoftDot(std::vector<uint8_t>& pixels, int cx, int cy, double radius, double feather, uint8_t r, uint8_t g,
                 uint8_t b, uint8_t a)
{
...
void DrawSoftRing(std::vector<uint8_t>& pixels, int cx, int cy, double radius, double thickness, double feather,
...
void DrawSoftLine(std::vector<uint8_t>& pixels, int x0, int y0, int x1, int y1, double radius, double feather,
                  uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
	const int reach = static_cast<int>(std::ceil(radius + feather));
	...
	for (int y = minY; y <= maxY; ++y) {
		for (int x = minX; x <= maxX; ++x) {
			const double dist =
			    DistanceToSegment(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5, static_cast<double>(x0),
                 ...
                 const SpatialStyle& style)
{
	...
	for (int y = 0; y < size; ++y) {
		...
		for (size_t i = 1; i < intersections.size(); i += 2) {
			const int xStart = std::clamp(static_cast<int>(std::ceil(intersections[i - 1])), 0, size - 1);
			const int xEnd = std::clamp(static_cast<int>(std::floor(intersections[i])), 0, size - 1);
...
void DrawBoundarySegment(std::vector<uint8_t>& pixels, const std::pair<int, int>& a, const std::pair<int, int>& b,
                         bool closing, double strokeRadius, double strokeFeather, const SpatialStyle& style)
{
	if (style.strokeMeters <= 0.0 || style.a == 0) return;
	const uint8_t haloAlpha = static_cast<uint8_t>(std::clamp(static_cast<int>(style.a) / 4, 0, 255));
	const uint8_t midAlpha = static_cast<uint8_t>(std::clamp(static_cast<int>(style.a), 0, 255));
	DrawSoftLine(pixels, a.first, a.second, b.first, b.second, closing ? strokeRadius * 0.9 : strokeRadius,
	             strokeFeather, style.r, style.g, style.b, haloAlpha);
	DrawSoftLine(pixels, a.first, a.second, b.first, b.second, closing ? strokeRadius * 0.35 : strokeRadius * 0.42,
	             std::max(1.5, strokeFeather * 0.35), style.r, style.g, style.b, midAlpha);
	DrawSoftLine(pixels, a.first, a.second, b.first, b.second, std::max(1.25, strokeRadius * 0.14), 1.5, 255, 255, 255,
	             static_cast<uint8_t>(std::min<int>(style.a, 92)));
}

void DrawBoundaryVertex(std::vector<uint8_t>& pixels, const std::pair<int, int>& p, bool first, bool last,
                        double dotRadius, const SpatialStyle& style)
{
	if (style.dotMeters <= 0.0 || style.a == 0) return;
	const uint8_t haloAlpha = style.fillA != 0 ? style.fillA : static_cast<uint8_t>(std::min<int>(style.a / 4, 72));
	if (last || first) {
		DrawSoftDot(pixels, p.first, p.second, dotRadius * 1.45, dotRadius * 0.55, style.r, style.g, style.b,
		            haloAlpha);
		DrawSoftRing(pixels, p.first, p.second, dotRadius, std::max(2.0, dotRadius * 0.25), 2.0, style.r, style.g,
		             style.b, static_cast<uint8_t>(std::min<int>(style.a, 240)));
		DrawSoftDot(pixels, p.first, p.second, dotRadius * 0.45, 2.0, style.r, style.g, style.b,
		            static_cast<uint8_t>(std::min<int>(style.a, 255)));
		return;
	}

	DrawSoftDot(pixels, p.first, p.second, dotRadius * 0.70, dotRadius * 0.30, style.r, style.g, style.b, haloAlpha);
	DrawSoftDot(pixels, p.first, p.second, dotRadius * 0.36, 1.75, style.r, style.g, style.b, style.a);
}
...
void HideOnly(const char* source)
{
	...
	if (s.visible) {
		openvr_pair::common::DiagnosticLog(
		    ...
		    s.uploadsDisabled ? 1 : 0, s.uploadFailureCount, static_cast<int>(s.lastError),
...
void Destroy()
{
	...
	if (vr::VROverlay()) {
		for (size_t i = 0; i < s.fileMarkerHandles.size(); ++i) {
			if (s.fileMarkerCreated[i] && s.fileMarkerHandles[i] != vr::k_ulOverlayHandleInvalid) {
...
vr::EVROverlayError UploadRasterTexture(const BoundaryPreviewRaster& raster)
{
	...
	if (raster.rgba.size() < static_cast<size_t>(BoundaryPreviewRaster::kTextureSize) *
	...
	constexpr GLsizei size = static_cast<GLsizei>(BoundaryPreviewRaster::kTextureSize);
	if (s.glTexture == 0) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size, size, 0, GL_RGBA, GL_UNSIGNED_BYTE, raster.rgba.data());
	}
	else {
		glBindTexture(GL_TEXTURE_2D, s.glTexture);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, size, size, GL_RGBA, GL_UNSIGNED_BYTE, raster.rgba.data());
	}
	...
	const vr::EVROverlayError err = vr::VROverlay()->SetOverlayTexture(s.handle, &tex);
	s.lastError = err;
	return err;
}
...
bool EnsureFileMarkerCreated(size_t index)
{
	...
	if (texturePath.empty()) {
		...
		if (s.fileMarkerFailureCount <= 3 || (s.fileMarkerFailureCount % 30) == 0) {
			openvr_pair::common::DiagnosticLog(
			    "boundary_preview_status",
			    "file_markers_visible=%d file_markers=%zu file_texture_ready=0 file_failures=%d file_error=%d "
			    "file_error_name=%s source=%s texture_missing=1",
			    s.fileMarkersVisible ? 1 : 0, s.fileMarkerCount, s.fileMarkerFailureCount,
			    static_cast<int>(s.fileMarkerLastError), OverlayErrorName(s.fileMarkerLastError), s.lastSource);
		}
		return false;
	}
	...
	vr::EVROverlayError err = vr::VROverlay()->CreateOverlay(key, name, &s.fileMarkerHandles[index]);
	if (err == vr::VROverlayError_KeyInUse) {
	...
	if (err != vr::VROverlayError_None) {
		...
		if (s.fileMarkerFailureCount <= 3 || (s.fileMarkerFailureCount % 30) == 0) {
			openvr_pair::common::DiagnosticLog(
			    "boundary_preview_status",
			    "file_marker_create_failed index=%zu error=%d error_name=%s failures=%d source=%s", index,
			    static_cast<int>(err), OverlayErrorName(err), s.fileMarkerFailureCount, s.lastSource);
		}
		return false;
	}
	...
	s.fileMarkerLastError = err;
	s.fileMarkerTextureReady = s.fileMarkerTextureReady || s.fileMarkerTextureReadyBySlot[index];
	if (err != vr::VROverlayError_None) {
		++s.fileMarkerFailureCount;
	}
	openvr_pair::common::DiagnosticLog(
	    "boundary_preview_status",
	    "file_marker_created index=%zu texture_ready=%d error=%d error_name=%s source=%s texture='%s'", index,
	    s.fileMarkerTextureReady ? 1 : 0, static_cast<int>(err), OverlayErrorName(err), s.lastSource,
	    texturePath.c_str());
...
void HideFileMarkers(const char* source)
{
	...
	openvr_pair::common::DiagnosticLog("boundary_preview_status",
	                                   "file_markers_visible=0 file_markers=0 file_texture_ready=%d file_failures=%d "
	                                   "file_error=%d file_error_name=%s source=%s",
	                                   s.fileMarkerTextureReady ? 1 : 0, s.fileMarkerFailureCount,
	                                   static_cast<int>(s.fileMarkerLastError), OverlayErrorName(s.fileMarkerLastError),
...
void TickFileMarkers(const std::vector<SpatialRenderCommand>& commands, const char* source)
{
	auto& s = State();
	const std::vector<BoundaryPreviewFileMarker> markers = BuildBoundaryPreviewFileMarkers(commands);
	if (markers.empty()) {
	...
	for (size_t i = 0; i < activeSlots; ++i) {
		...
		vr::VROverlay()->SetOverlayWidthInMeters(s.fileMarkerHandles[i], MarkerWidthMeters(marker.style));
		const vr::HmdMatrix34_t mat = BoundaryPreviewFileMarkerTransform(marker.vertex);
		vr::EVROverlayError err =
		    vr::VROverlay()->SetOverlayTransformAbsolute(s.fileMarkerHandles[i], BoundaryPreviewTrackingOrigin(), &mat);
		if (err == vr::VROverlayError_None) {
			err = vr::VROverlay()->ShowOverlay(s.fileMarkerHandles[i]);
		}
		if (err != vr::VROverlayError_None) {
			if (vr::VROverlay() && s.fileMarkerHandles[i] != vr::k_ulOverlayHandleInvalid) {
				vr::VROverlay()->HideOverlay(s.fileMarkerHandles[i]);
			}
			s.fileMarkerLastError = err;
			++s.fileMarkerFailureCount;
			if (s.fileMarkerFailureCount <= 3 || (s.fileMarkerFailureCount % 30) == 0) {
				openvr_pair::common::DiagnosticLog("boundary_preview_status",
				                                   "file_marker_show_failed index=%zu error=%d error_name=%s "
				                                   "failures=%d source=%s pos=(%.3f,%.3f,%.3f)",
				                                   i, static_cast<int>(err), OverlayErrorName(err),
				                                   s.fileMarkerFailureCount, source ? source : s.lastSource,
				                                   marker.vertex.x, marker.vertex.y, marker.vertex.z);
			}
			continue;
		}
		++shown;
	...
	if (s.fileMarkersVisible != wasVisible || previousCount != shown) {
		openvr_pair::common::DiagnosticLog(
		    "boundary_preview_status",
		    "file_markers_visible=%d file_markers=%zu file_texture_ready=%d file_failures=%d file_error=%d "
		    "file_error_name=%s source=%s raw_visible=%d raw_disabled=%d",
		    s.fileMarkersVisible ? 1 : 0, s.fileMarkerCount, s.fileMarkerTextureReady ? 1 : 0, s.fileMarkerFailureCount,
		    static_cast<int>(s.fileMarkerLastError), OverlayErrorName(s.fileMarkerLastError),
		    source ? source : s.lastSource, s.visible ? 1 : 0, s.uploadsDisabled ? 1 : 0);
	}
}
...
{
	const double pixels =
	    meters / std::max(spanMeters, 0.001) * static_cast<double>(BoundaryPreviewRaster::kTextureSize - 1);
...
BoundaryPreviewPlane ComputeBoundaryPreviewPlane(const std::vector<SpatialRenderCommand>& commands)
{
	...
	for (const SpatialRenderCommand& command : commands) {
		allVertices.insert(allVertices.end(), command.standingVertices.begin(), command.standingVertices.end());
	}
...
{
	SpatialSession session = BoundaryCaptureSessionDescriptor(StandingSpace(), -1, {},
	                                                          vertices.empty() ? 0.0 : vertices.front().y, false, 0);
...
BoundaryPreviewRaster BuildBoundaryPreviewRaster(const std::vector<SpatialRenderCommand>& commands)
{
	...
	raster.rgba.assign(static_cast<size_t>(BoundaryPreviewRaster::kTextureSize) *
	                       static_cast<size_t>(BoundaryPreviewRaster::kTextureSize) * 4u,
	                   0u);
	...
	const double scale = static_cast<double>(BoundaryPreviewRaster::kTextureSize - 1) / raster.plane.spanMeters;
	...
	for (const SpatialRenderCommand& command : commands) {
		...
		const double strokeRadius =
		    command.style.strokeMeters > 0.0
		        ? WorldMetersToPixels(raster.plane.spanMeters, command.style.strokeMeters, 2.0, 18.0)
		        : 0.0;
		const double strokeFeather =
		    command.style.strokeMeters > 0.0
		        ? WorldMetersToPixels(raster.plane.spanMeters, command.style.strokeMeters * 0.40, 1.5, 8.0)
		        : 0.0;
		const double dotRadius = command.style.dotMeters > 0.0
		                             ? WorldMetersToPixels(raster.plane.spanMeters, command.style.dotMeters, 4.0, 22.0)
		                             : 0.0;
		...
		for (size_t i = 1; i < vertices.size(); ++i) {
			...
			DrawBoundarySegment(raster.rgba, pixelPoints[i - 1], pixelPoints[i], false, strokeRadius, strokeFeather,
			                    segmentStyle);
		}
		if (command.closeLoop && vertices.size() >= 3) {
			DrawBoundarySegment(raster.rgba, pixelPoints.back(), pixelPoints.front(), true, strokeRadius, strokeFeather,
			                    command.style);
		}
		...
		for (size_t i = 0; i < vertices.size(); ++i) {
			...
			DrawBoundaryVertex(raster.rgba, pixelPoints[i], i == 0, i + 1 == vertices.size(), dotRadius, vertexStyle);
		}
	}
	return raster;
}
...
BuildBoundaryPreviewFileMarkers(const std::vector<SpatialRenderCommand>& commands)
{
	...
	std::vector<BoundaryPreviewFileMarker>& source = explicitMarkers.empty() ? pathMarkers : explicitMarkers;
	...
	for (int i = 0; i < kMaxFileMarkerOverlays; ++i) {
		const double t = static_cast<double>(i) / static_cast<double>(kMaxFileMarkerOverlays - 1);
...
void TickBoundaryPreview(bool wantVisible, const std::vector<BoundaryVertex>& vertices, double floorY, bool closeLoop,
                         const char* source)
{
	SpatialSession session = BoundaryCaptureSessionDescriptor(StandingSpace(), -1, {}, floorY, false, 0);
	SpatialPrimitive primitive = BoundaryPathPrimitive(session, vertices, closeLoop);
	TickBoundaryPreview(wantVisible, BuildSpatialRenderCommands({primitive}), floorY, source);
}

void TickBoundaryPreview(bool wantVisible, const std::vector<SpatialRenderCommand>& commands, double floorY,
                         const char* source, bool showFileMarkers)
{
	auto& s = State();
	...
	if (contentChanged && NowSeconds() >= s.nextUploadAttemptSec) {
		...
		if (err == vr::VROverlayError_None) {
			...
			s.nextUploadAttemptSec = NowSeconds() + BoundaryUploadBackoffSeconds(s.uploadFailureCount);
			if (s.uploadFailureCount <= 3 || (s.uploadFailureCount % 30) == 0) {
				openvr_pair::common::DiagnosticLog(
				    "boundary-preview",
				    "texture_upload_failed err=%d error_name=%s count=%d source=%s vertices=%zu hash=%llu backoff=%.3f "
				    "mode=opengl_texture",
				    static_cast<int>(err), OverlayErrorName(err), s.uploadFailureCount, s.lastSource, s.lastVertexCount,
				    static_cast<unsigned long long>(raster.hash), BoundaryUploadBackoffSeconds(s.uploadFailureCount));
			}
		}
	}
	...
	vr::VROverlay()->SetOverlayWidthInMeters(s.handle, static_cast<float>(raster.plane.spanMeters));
	vr::HmdMatrix34_t mat = BoundaryPreviewTransform(raster.plane.centerX, floorY, raster.plane.centerZ);
	vr::VROverlay()->SetOverlayTransformAbsolute(s.handle, BoundaryPreviewTrackingOrigin(), &mat);
	vr::VROverlay()->ShowOverlay(s.handle);
	if (!s.visible) {
		openvr_pair::common::DiagnosticLog(
		    "boundary_preview_status",
		    "created=1 visible=1 uploads_disabled=0 failures=%d error=%d error_name=%s source=%s vertices=%zu "
		    "span=%.3f center=(%.3f,%.3f) mode=opengl_texture texture_ready=%d",
		    s.uploadFailureCount, static_cast<int>(s.lastError), OverlayErrorName(s.lastError), s.lastSource,
		    s.lastVertexCount, raster.plane.spanMeters, raster.plane.centerX, raster.plane.centerZ,
		    s.textureReady ? 1 : 0);
	}
	s.visible = true;
}
