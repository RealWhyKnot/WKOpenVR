	...
	double now)
{
	...
	const double ready = spacecal::calibration_progress::OneShotReadyScore(
		ctx.state,
		sampleProgress,
		sampleTarget,
		rotationDiversity,
		translationDiversity);
	...
	const bool rotationGatePending =
		ctx.state == CalibrationState::Rotation &&
		sampleFilled &&
	...
	const char* gate = gatePending
		? "waiting_motion"
		: (sampleFilled ? "ready_or_solving" : "collecting_samples");
	char buf[320];
	snprintf(buf, sizeof buf,
		"[oneshot] readiness phase=%s ready_pct=%d samples=%d/%d rot=%.2f/%.2f tr=%.2f/%.2f gate=%s",
		PhaseName(ctx.state),
		(int)(clampedReady * 100.0),
		sampleProgress,
		sampleTarget,
		rotationDiversity,
		spacecal::calibration_progress::kOneShotRotationReadyDiversity,
		translationDiversity,
		spacecal::calibration_progress::kOneShotTranslationReadyDiversity,
		gate);
