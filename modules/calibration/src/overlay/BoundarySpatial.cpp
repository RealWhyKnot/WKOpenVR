...
SpatialSpace TargetSpace(std::string trackingSystem, const Eigen::AffineCompact3d& targetToStanding,
...
SpatialSession BoundaryCaptureSessionDescriptor(const SpatialSpace& authoringSpace, int32_t sourceDeviceId,
                                                std::string sourceTrackingSystem, double floorY, bool requireTrigger,
                                                uint64_t sessionId)
{
...
SpatialPrimitive BoundaryPathPrimitive(const SpatialSession& session, const std::vector<BoundaryVertex>& vertices,
                                       bool closeLoop)
{
	SpatialPrimitive primitive;
	primitive.kind = closeLoop ? SpatialPrimitiveKind::PolygonFloorRegion : SpatialPrimitiveKind::PolylinePath;
...
SpatialPrimitive FloorMarkerPrimitive(const SpatialSpace& space, const std::vector<BoundaryVertex>& vertices,
                                      double floorY)
{
	SpatialPrimitive primitive;
...
SpatialPrimitive TransformPrimitiveToStanding(const SpatialPrimitive& primitive)
{
	...
	out.vertices = TransformToStandingUniverse(primitive.vertices, primitive.space.targetToStanding);
	out.floorY =
	    TransformHeightToStandingUniverse(primitive.vertices, primitive.floorY, primitive.space.targetToStanding);
	out.ceilingY =
	    TransformHeightToStandingUniverse(primitive.vertices, primitive.ceilingY, primitive.space.targetToStanding);
	out.space = StandingSpace(primitive.space.trackingSystem);
	out.space.transformEpoch = primitive.space.transformEpoch;
	return out;
}

std::vector<SpatialRenderCommand> BuildSpatialRenderCommands(const std::vector<SpatialPrimitive>& primitives)
{
	...
	std::stable_sort(commands.begin(), commands.end(),
	                 [](const SpatialRenderCommand& a, const SpatialRenderCommand& b) { return a.layer < b.layer; });
	return commands;
}

std::vector<SpatialRenderCommand> BuildPersistentBoundaryCommands(const std::vector<BoundaryVertex>& standingVertices,
...
uint8_t BoundaryAgeShade(size_t vertexIndex, size_t vertexCount)
{
	...
	const double t = static_cast<double>(vertexIndex) / static_cast<double>(vertexCount - 1);
