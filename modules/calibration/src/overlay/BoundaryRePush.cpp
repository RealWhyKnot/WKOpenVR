...
static bool TransformDeltaExceedsThreshold(const Eigen::AffineCompact3d& current, const Eigen::AffineCompact3d& last)
{
...
static void DoPush(const Eigen::AffineCompact3d& xf)
{
	...
	auto standing = bc.standingSpace ? bc.vertices : wkopenvr::boundary::TransformToStandingUniverse(bc.vertices, xf);
	const double standingFloorY =
	    bc.standingSpace ? bc.floorY
	                     : wkopenvr::boundary::TransformHeightToStandingUniverse(bc.vertices, bc.floorY, xf);
	const double standingCeilingY =
	    bc.standingSpace ? bc.ceilingY
	                     : wkopenvr::boundary::TransformHeightToStandingUniverse(bc.vertices, bc.ceilingY, xf);
	const bool ok = wkopenvr::boundary::PushToChaperone(standing, standingFloorY, standingCeilingY);
	if (ok) {
		if (g_firstPush) {
			// Initial push after profile load.
			char lbuf[80];
			snprintf(lbuf, sizeof lbuf, "[boundary-re-push] initial push: vertices=%zu", bc.vertices.size());
		...
		Metrics::chaperoneRePushCount.Push(Metrics::chaperoneRePushCount.size() > 0
...
void NoteBoundaryPushedForTransform(const Eigen::AffineCompact3d& targetToStanding)
{
	...
	char lbuf[192];
	snprintf(lbuf, sizeof lbuf, "[boundary-re-push] synchronized direct push transform: trans=(%.3f,%.3f,%.3f)", t.x(),
	         t.y(), t.z());
	Metrics::WriteLogAnnotation(lbuf);
}
...
void TickBoundaryRePush(double /*time*/)
{
	...
	const bool pushable = bc.enabled && !bc.vertices.empty() && ctx.validProfile;
	...
	const boundary_repush::StartupCountdown sc = boundary_repush::StepStartupCountdown(pushable, g_startupPushPending);
	...
	if (!g_firstPush) {
		const double elapsedSec = std::chrono::duration<double>(now - g_lastPushAt).count();
	...
	if (!g_firstPush && !kDynamicRePushEnabled) {
		...
		if (!g_loggedSkipSincePush && TransformDeltaExceedsThreshold(xf, g_lastPushedTransform)) {
...

	{
		...
		char lbuf[224];
		snprintf(lbuf, sizeof lbuf, "[boundary-re-push] push using transform: trans=(%.3f,%.3f,%.3f) vertices=%zu",
