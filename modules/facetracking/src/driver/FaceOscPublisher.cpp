...
static_assert(
    sizeof(kUpstreamExpressionNames) / sizeof(kUpstreamExpressionNames[0]) ==
...
{
    return Upstream(frame, U_BrowOuterUpRight) * 0.60f +
           Upstream(frame, U_BrowInnerUpRight) * 0.40f;
}

static float BrowUpLeft(const protocol::FaceTrackingFrameBody &frame)
{
    return Upstream(frame, U_BrowOuterUpLeft) * 0.60f +
           Upstream(frame, U_BrowInnerUpLeft) * 0.40f;
}

static float BrowDownRight(const protocol::FaceTrackingFrameBody &frame)
{
    return Upstream(frame, U_BrowLowererRight) * 0.75f +
           Upstream(frame, U_BrowPinchRight) * 0.25f;
}

static float BrowDownLeft(const protocol::FaceTrackingFrameBody &frame)
{
    return Upstream(frame, U_BrowLowererLeft) * 0.75f +
           Upstream(frame, U_BrowPinchLeft) * 0.25f;
}

static float MouthSmileRight(const protocol::FaceTrackingFrameBody &frame)
{
    return Upstream(frame, U_MouthCornerPullRight) * 0.80f +
           Upstream(frame, U_MouthCornerSlantRight) * 0.20f;
}

static float MouthSmileLeft(const protocol::FaceTrackingFrameBody &frame)
{
    return Upstream(frame, U_MouthCornerPullLeft) * 0.80f +
           Upstream(frame, U_MouthCornerSlantLeft) * 0.20f;
}

static float MouthSadRight(const protocol::FaceTrackingFrameBody &frame)
{
    return std::max(Upstream(frame, U_MouthFrownRight),
                    Upstream(frame, U_MouthStretchRight));
}

static float MouthSadLeft(const protocol::FaceTrackingFrameBody &frame)
{
    return std::max(Upstream(frame, U_MouthFrownLeft),
                    Upstream(frame, U_MouthStretchLeft));
}
...
static OscCounts PublishCurrentVrcft(const protocol::FaceTrackingFrameBody &frame)
{
    ...
    PublishFtV2Float(counts, "EyeLidLeft",
        open_l * 0.75f + Upstream(frame, U_EyeWideLeft) * 0.25f);
    PublishFtV2Float(counts, "EyeLidRight",
        open_r * 0.75f + Upstream(frame, U_EyeWideRight) * 0.25f);
    ...
    PublishFtV2Float(counts, "EyeWide",
        std::max(Upstream(frame, U_EyeWideLeft), Upstream(frame, U_EyeWideRight)));
    ...
    const float browExpressionRight =
        std::min(1.0f, Avg2(Upstream(frame, U_BrowInnerUpRight),
                            Upstream(frame, U_BrowOuterUpRight))) - browDownRight;
    const float browExpressionLeft =
        std::min(1.0f, Avg2(Upstream(frame, U_BrowInnerUpLeft),
                            Upstream(frame, U_BrowOuterUpLeft))) - browDownLeft;
    ...
    PublishFtV2Float(counts, "BrowExpression",
        Avg2(browExpressionRight, browExpressionLeft));

    PublishFtV2Float(counts, "JawX",
        Upstream(frame, U_JawRight) - Upstream(frame, U_JawLeft));
    PublishFtV2Float(counts, "JawZ",
        Upstream(frame, U_JawForward) - Upstream(frame, U_JawBackward));

    PublishFtV2Float(counts, "CheekSquint",
        Avg2(Upstream(frame, U_CheekSquintLeft), Upstream(frame, U_CheekSquintRight)));
    PublishFtV2Float(counts, "CheekPuffSuckLeft",
        Upstream(frame, U_CheekPuffLeft) - Upstream(frame, U_CheekSuckLeft));
    ...
    PublishFtV2Float(counts, "CheekSuck",
        Avg2(Upstream(frame, U_CheekSuckLeft), Upstream(frame, U_CheekSuckRight)));

    PublishFtV2Float(counts, "MouthUpperX",
        Upstream(frame, U_MouthUpperRight) - Upstream(frame, U_MouthUpperLeft));
    PublishFtV2Float(counts, "MouthLowerX",
        Upstream(frame, U_MouthLowerRight) - Upstream(frame, U_MouthLowerLeft));
    ...
    const float lipSuckUpper = Avg2(Upstream(frame, U_LipSuckUpperRight),
                                    Upstream(frame, U_LipSuckUpperLeft));
    const float lipSuckLower = Avg2(Upstream(frame, U_LipSuckLowerRight),
                                    Upstream(frame, U_LipSuckLowerLeft));
    const float lipFunnelUpper = Avg2(Upstream(frame, U_LipFunnelUpperRight),
                                      Upstream(frame, U_LipFunnelUpperLeft));
    const float lipFunnelLower = Avg2(Upstream(frame, U_LipFunnelLowerRight),
                                      Upstream(frame, U_LipFunnelLowerLeft));
    const float lipPuckerUpper = Avg2(Upstream(frame, U_LipPuckerUpperRight),
                                      Upstream(frame, U_LipPuckerUpperLeft));
    const float lipPuckerLower = Avg2(Upstream(frame, U_LipPuckerLowerRight),
                                      Upstream(frame, U_LipPuckerLowerLeft));
    ...
    const float mouthUpperUp = Avg2(Upstream(frame, U_MouthUpperUpRight),
                                    Upstream(frame, U_MouthUpperUpLeft));
    const float mouthLowerDown = Avg2(Upstream(frame, U_MouthLowerDownRight),
                                      Upstream(frame, U_MouthLowerDownLeft));
    const float mouthStretch = Avg2(Upstream(frame, U_MouthStretchRight),
                                    Upstream(frame, U_MouthStretchLeft));
    const float mouthTightener = Avg2(Upstream(frame, U_MouthTightenerRight),
                                      Upstream(frame, U_MouthTightenerLeft));
    ...
    PublishFtV2Float(counts, "MouthPress",
        Avg2(Upstream(frame, U_MouthPressRight), Upstream(frame, U_MouthPressLeft)));
    PublishFtV2Float(counts, "MouthDimple",
        Avg2(Upstream(frame, U_MouthDimpleRight), Upstream(frame, U_MouthDimpleLeft)));
    PublishFtV2Float(counts, "NoseSneer",
        Avg2(Upstream(frame, U_NoseSneerRight), Upstream(frame, U_NoseSneerLeft)));
    ...
    PublishFtV2Float(counts, "SmileFrownRight",
        smileRight - Upstream(frame, U_MouthFrownRight));
    PublishFtV2Float(counts, "SmileFrownLeft",
        smileLeft - Upstream(frame, U_MouthFrownLeft));
    ...
    PublishFtV2Float(counts, "TongueX",
        Upstream(frame, U_TongueRight) - Upstream(frame, U_TongueLeft));
    PublishFtV2Float(counts, "TongueY",
        Upstream(frame, U_TongueUp) - Upstream(frame, U_TongueDown));
    PublishFtV2Float(counts, "TongueArchY",
        Upstream(frame, U_TongueCurlUp) - Upstream(frame, U_TongueBendDown));
    PublishFtV2Float(counts, "TongueShape",
        Upstream(frame, U_TongueFlat) - Upstream(frame, U_TongueSquish));
