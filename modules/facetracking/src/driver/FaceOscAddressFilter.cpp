...
static std::string CompatibleKey(const std::string& address)
{
	...
	std::string body = StartsWith(address, kAvatarPrefix) ? address.substr(sizeof(kAvatarPrefix) - 1) : address;
...
FaceOscAddressFilter::FaceOscAddressFilter(std::wstring path) : path_(std::move(path)) {}

const char* FaceOscAddressFilterLoadStatusName(FaceOscAddressFilterLoadStatus status)
{
	switch (status) {
		case FaceOscAddressFilterLoadStatus::NotConfigured:
			return "not-configured";
		case FaceOscAddressFilterLoadStatus::Missing:
			return "missing";
		case FaceOscAddressFilterLoadStatus::ReadFailed:
			return "read-failed";
		case FaceOscAddressFilterLoadStatus::Loaded:
			return "loaded";
		default:
			return "unknown";
	}
