...
void FacetrackingPlugin::PushConfigToDriver()
{
	...
	try {
		...
		FT_LOG_OVL("[ipc] config pushed: osc_enabled=%d active_module='%s' eyelid=%d/%d/mode=%d "
		           "vergence=%d/%d corr=0x%02x corr_strengths=0x%04x "
		           ...
		           (int)cfg.vergence_lock_strength, (int)cfg.expression_correction_flags,
		           (int)cfg.expression_correction_strengths, (int)cfg.gaze_smoothing, (int)cfg.openness_smoothing,
		           (int)cfg.eye_close_assist_enabled, (int)cfg.eye_close_assist_strength);
		PushShapeTuningToDriver();
	}
	catch (const std::exception& e) {
