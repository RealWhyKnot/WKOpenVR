...
void DrawAdvancedTab(FacetrackingPlugin& plugin)
{
	...
	if (SliderIntWithTooltip("Gaze smoothing", &p.gaze_smoothing, 0, 100, "%d%%",
	...
	if (SliderIntWithTooltip("Openness smoothing", &p.openness_smoothing, 0, 100, "%d%%",
	...
	if (!hs.valid) {
		...
		int m = (s % 3600) / 60;
		int sec = s % 60;
		ImGui::TextColored(pal.statusOk, "Host running: pid=%d uptime=%02d:%02d:%02d", hs.host_pid, h, m, sec);
		if (!hs.phase.empty()) {
		...
		ImGui::TextDisabled("Host frames written: %llu", (unsigned long long)hs.frames_written);
		const auto& dt = plugin.driver_telemetry_.Snapshot();
		if (dt.valid) {
			ImGui::TextDisabled("Driver frames read: %llu | OSC sent: %llu | dropped: %llu",
			                    (unsigned long long)dt.frames_read, (unsigned long long)dt.osc_messages_sent,
			                    (unsigned long long)dt.osc_messages_dropped);
		}
		if (hs.active_module.has_value()) {
			ImGui::Text("Active module: %s %s (%s)", hs.active_module->name.c_str(), hs.active_module->version.c_str(),
