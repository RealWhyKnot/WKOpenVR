...
struct ModulesTabState
{
	...
	// Module whose settings panel is expanded, and the schema/values behind it.
	// Values are held here between frames so typing into a text field does not
	// rewrite the file on every keystroke; a write happens on commit.
	std::string settings_uuid;
	ModuleSettingsSchema settings_schema;
	picojson::object settings_values;
...
static void RefreshIfStale()
{
	...
	if (now - static_cast<uint64_t>(g_state.last_scan_tick) < 1000 && g_state.initialised) return;
...
static std::string PickFolder()
{
	...
	if (FAILED(CoCreateInstance(CLSID_FileOpenDialog, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pDlg)))) return {};
...
static std::string ParseOwnerRepo(const std::string& input)
{
	...
	if (input.rfind(prefix, 0) == 0) return input.substr(sizeof(prefix) - 1);
	// Rudimentary validation: must contain exactly one '/'.
	size_t slash = input.find('/');
	if (slash == std::string::npos || slash == 0 || slash == input.size() - 1) return {};
	return input;
}
...
static std::string BuildSourceDataJson(const ModuleSource& src)
{
	...
	o["kind"] = picojson::value(src.kind == SourceKind::Folder   ? "folder"
...
static std::string BuildRegistryInstallDataJson(const ModuleSource& src, const AvailableModule& mod)
{
	picojson::object o;
	o["id"] = picojson::value(src.id);
...
static std::string ResolveSourceLabel(const InstalledModule& m, const SourcesCatalogue& cat)
{
	...
	if (!m.source_kind_str.empty()) {
		...
		if (m.source_kind_str == "github")
			return m.release_tag.empty() ? std::string{"GitHub"} : "GitHub @ " + m.release_tag;
...
static bool IsModuleEnabled(const FacetrackingPlugin& plugin, const std::string& uuid)
{
...
static void DisableModulesFromSource(FacetrackingPlugin& plugin, const std::string& sourceId)
{
	...
	if (next.size() != before) {
		FT_LOG_OVL("[modules] disabling %zu module(s) before removing source '%s'", before - next.size(),
		           sourceId.c_str());
...
static void CloseModuleSettings()
{
	g_state.settings_uuid.clear();
	g_state.settings_schema = {};
	g_state.settings_values.clear();
	g_state.settings_path.clear();
	g_state.settings_error.clear();
}

static void OpenModuleSettings(const InstalledModule& m)
{
	CloseModuleSettings();
	g_state.settings_schema = LoadModuleSettingsSchema(m.manifest_path);
	if (!g_state.settings_schema.loaded) return;

	g_state.settings_uuid = m.uuid;
	g_state.settings_path = ModuleSettingsValuesPath(g_state.settings_schema.values_file);
	if (g_state.settings_path.empty()) {
		g_state.settings_error = "This module declares an unusable settings file name.";
		return;
	}
	if (!ReadModuleSettingsValues(g_state.settings_path, g_state.settings_values)) {
		g_state.settings_error = "Could not read " + g_state.settings_path + " -- showing declared defaults.";
	}
}

static void PersistModuleSettings()
{
	if (WriteModuleSettingsValues(g_state.settings_path, g_state.settings_values)) {
		g_state.settings_error.clear();
		FT_LOG_OVL("[modules] settings written: %s", g_state.settings_path.c_str());
		return;
	}
	g_state.settings_error = "Could not write " + g_state.settings_path;
	FT_LOG_OVL("[modules] settings write FAILED: %s", g_state.settings_path.c_str());
}
...
static bool SettingBool(const ModuleSettingSpec& spec)
{
	auto it = g_state.settings_values.find(spec.key);
	if (it != g_state.settings_values.end() && it->second.is<bool>()) return it->second.get<bool>();
	return spec.default_bool;
}

static double SettingNumber(const ModuleSettingSpec& spec)
{
	auto it = g_state.settings_values.find(spec.key);
	if (it != g_state.settings_values.end() && it->second.is<double>()) return it->second.get<double>();
	return spec.default_number;
}

static std::string SettingString(const ModuleSettingSpec& spec)
{
	auto it = g_state.settings_values.find(spec.key);
	if (it != g_state.settings_values.end() && it->second.is<std::string>()) return it->second.get<std::string>();
	return spec.default_string;
}

static void DrawModuleSettingsPanel()
{
	if (g_state.settings_uuid.empty()) return;

	const InstalledModule* mod = FindInstalledByUuid(g_state.settings_uuid);
	if (!mod) {
		CloseModuleSettings();
		return;
	}
	...
	ImGui::Separator();
	DrawSectionHeading((mod->name + " settings").c_str());

	if (!g_state.settings_error.empty()) {
		ImGui::TextColored(pal.statusError, "%s", g_state.settings_error.c_str());
	}

	ImGui::TextDisabled("Saved to %s", g_state.settings_path.c_str());
	...
	bool dirty = false;
	for (const ModuleSettingSpec& spec : g_state.settings_schema.settings) {
		ImGui::PushID(spec.key.c_str());

		if (spec.type == "bool") {
			bool v = SettingBool(spec);
			if (ImGui::Checkbox(spec.label.c_str(), &v)) {
				g_state.settings_values[spec.key] = picojson::value(v);
				dirty = true;
			}
		}
		else if (spec.type == "int") {
			int v = static_cast<int>(SettingNumber(spec));
			...
			if (ImGui::SliderInt(spec.label.c_str(), &v, lo, hi)) {
				g_state.settings_values[spec.key] = picojson::value(static_cast<double>(v));
				dirty = true;
			}
		}
		else if (spec.type == "float") {
			float v = static_cast<float>(SettingNumber(spec));
			...
			const std::string current = SettingString(spec);
			int index = 0;
			...
			if (ImGui::BeginCombo(spec.label.c_str(), spec.options[static_cast<size_t>(index)].c_str())) {
				for (size_t i = 0; i < spec.options.size(); ++i) {
					...
					if (ImGui::Selectable(spec.options[i].c_str(), selected)) {
						g_state.settings_values[spec.key] = picojson::value(spec.options[i]);
						dirty = true;
					}
			...
			if (ImGui::InputText(spec.label.c_str(), buf, sizeof(buf),
				...
				g_state.settings_values[spec.key] = picojson::value(std::string(buf));
				dirty = true;
			}
	...
	if (dirty) PersistModuleSettings();

	if (ImGui::SmallButton("Close##ft_settings_close")) CloseModuleSettings();
}
...
static void DrawInstalledModulesSection(FacetrackingPlugin& plugin)
{
	...
	const std::string* hostActiveUuid = hs.valid && hs.active_module.has_value() ? &hs.active_module->uuid : nullptr;
	...
	{
		TableScope installedTable("ft_installed_v4", 6, tf);
		...
		SetupFixedColumn("Settings", 90.0f);
		DrawTableHeader();
		...
		for (size_t i = 0; i < g_state.installed.size(); ++i) {
			...
			// Column 5: settings toggle, shown only for modules that ship a descriptor.
			ImGui::TableSetColumnIndex(5);
			const bool open = g_state.settings_uuid == m.uuid;
			if (m.has_settings) {
				const std::string btnId = (open ? "Hide##ft_set_" : "Settings##ft_set_") + m.uuid;
				...
				ImGui::TextDisabled("--");
				TooltipForLastItem("This module does not publish a settings descriptor.");
			}
		}
	}
...
static void DrawSourcesSection(FacetrackingPlugin& plugin)
{
	...
	if (!g_state.sync_status.empty()) {
		...
		if (g_state.sync_status_ok)
			ImGui::TextColored(palSync.statusOk, "Sync: %s", g_state.sync_status.c_str());
		else
			ImGui::TextColored(palSync.statusError, "Sync error: %s", g_state.sync_status.c_str());
	}
	...
	if (ImGui::Button("Add folder source...##ft_add_folder")) {
		...
		if (!chosen.empty()) {
			...
			src.label = "Folder: " + (slash != std::string::npos ? chosen.substr(slash + 1) : chosen);
	...
	ImGui::SameLine();
	if (ImGui::Button("Add GitHub repo...##ft_add_github")) ImGui::OpenPopup("ft_add_github_popup");
	TooltipForLastItem("Fetch the latest release from a GitHub repository.");
	ImGui::EndDisabled();

	if (ImGui::BeginPopupModal("ft_add_github_popup", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		ImGui::Text("GitHub repository URL or owner/repo:");
		ImGui::SetNextItemWidth(360.0f);
		ImGui::InputText("##ft_github_url", g_state.github_input, sizeof(g_state.github_input));
		...
		ImGui::SameLine();
		if (ImGui::Button("Cancel##ft_github_cancel", ImVec2(110, 0))) ImGui::CloseCurrentPopup();
		ImGui::EndPopup();
	}
}
...
static void DrawTrustBanners()
{
	...
	if (hasNonRegistry) {
		DrawBanner("Untrusted sources",
		           ...
		           ImVec4(0.38f, 0.30f, 0.02f, 1.0f), ImVec4(1.0f, 0.92f, 0.45f, 1.0f),
		           ImVec4(1.0f, 0.96f, 0.80f, 1.0f));
	}

	if (hasUnverifiedGithub) {
		DrawBanner("Unverified SHA-256",
		           ...
		           ImVec4(0.42f, 0.06f, 0.06f, 1.0f), ImVec4(1.0f, 0.88f, 0.88f, 1.0f),
