...
bool CreateMapping(const char* name, HANDLE& mapping_out, Layout*& layout_out,
                   uint32_t magic)
{
    mapping_out = CreateFileMappingA(
        INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(Layout), name);
    if (!mapping_out) return false;
    layout_out = reinterpret_cast<Layout*>(
        MapViewOfFile(mapping_out, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(Layout)));
    if (!layout_out) { CloseHandle(mapping_out); mapping_out = nullptr; return false; }
...
template <typename Layout>
bool OpenMapping(const char* name, HANDLE& mapping_out, Layout*& layout_out,
                 DWORD access)
{
    mapping_out = OpenFileMappingA(access, FALSE, name);
    if (!mapping_out) return false;
    layout_out = reinterpret_cast<Layout*>(
        MapViewOfFile(mapping_out, access, 0, 0, sizeof(Layout)));
    if (!layout_out) { CloseHandle(mapping_out); mapping_out = nullptr; return false; }
    return true;
}

template <typename Layout>
void CloseMapping(HANDLE& mapping, Layout*& layout)
{
    if (layout)  { UnmapViewOfFile(layout); layout = nullptr; }
