...
int main(int argc, char* argv[])
{
	...
	if (self_test) {
		std::fprintf(stderr,
		             ...
		             phantom::kPhantomInferenceShmemMagic, phantom::kPhantomInferenceShmemVersion,
		             (unsigned)phantom::kBodyRoleCount, sizeof(phantom::PhantomInferenceInLayout),
		...
		if (!L || L->magic != (phantom::kPhantomInferenceShmemMagic ^ 0xFFFF) ||
	...
	int attempt = 0;
	while (!g_stop.load(std::memory_order_acquire) && !in.Open(OPENVR_PAIRDRIVER_PHANTOM_INFERENCE_IN_SHMEM_NAME)) {
