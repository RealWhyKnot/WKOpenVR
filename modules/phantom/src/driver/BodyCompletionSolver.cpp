...
void SetRole(BodyCompletionResult& result,
             BodyRole role,
             const BodyCompletionPose& pose,
             float confidence,
             uint16_t source,
             BodyCompletionMode mode,
             uint32_t age_ms = 0)
{
...
BodyCompletionPose OffsetPose(const BodyCompletionPose& base,
...
BodyCompletionPose MidPose(const BodyCompletionPose& a,
                           const BodyCompletionPose& b,
                           const double forward[3],
                           double forward_bias,
                           const double rot[4])
{
...
float DegradeByReach(const BodyCompletionPose& a,
                     const BodyCompletionPose& b,
                     double max_reach,
...
bool ShouldPlantFoot(const BodyCompletionSensorPose& hmd, double floor_y)
{
    ...
    return PlanarLen3(hmd.pose.velocity) < 0.20
        && std::abs(hmd.pose.velocity[1]) < 0.15
        && hmd.pose.position[1] > floor_y + 0.60;
}

BodyCompletionPose ApplyFootLock(BodyCompletionFootLockState& lock,
                                 const BodyCompletionPose& target,
                                 bool can_plant,
                                 bool& held)
{
    held = false;
...
{
    switch (mode) {
    case BodyCompletionMode::Measured:      return 1.0;
    case BodyCompletionMode::ControllerIk:  return 0.55;
    case BodyCompletionMode::HmdRoot:       return 0.45;
    case BodyCompletionMode::FloorContact:  return 0.35;
    case BodyCompletionMode::HeldContact:   return 0.25;
    case BodyCompletionMode::LowConfidence: return 0.25;
    case BodyCompletionMode::None:          return 1.0;
    }
    return 1.0;
}

void SmoothOutput(BodyCompletionRoleOutput& out,
                  BodyCompletionSolverRoleState& prev)
{
    if (!out.valid) return;
    ...
    out.confidence = static_cast<float>(
        prev.confidence + (out.confidence - prev.confidence) * alpha);
    prev.pose = out.pose;
    prev.confidence = out.confidence;
    prev.valid = true;
}
...
{
    switch (mode) {
    case BodyCompletionMode::None:          return "none";
    case BodyCompletionMode::Measured:      return "measured";
    case BodyCompletionMode::HmdRoot:       return "hmd_root";
    case BodyCompletionMode::ControllerIk:  return "controller_ik";
    case BodyCompletionMode::FloorContact:  return "floor_contact";
    case BodyCompletionMode::HeldContact:   return "held_contact";
    case BodyCompletionMode::LowConfidence: return "low_confidence";
    }
    return "unknown";
}
...
BodyCompletionResult BodyCompletionSolver::Solve(const BodyCompletionInput& input)
{
    ...
    const double head_height = Clamp(input.hmd.pose.position[1] - cal.floor_y_m,
        0.55, cal.height_m + 0.25);
    const double crouch_scale = Clamp(head_height / cal.height_m, 0.45, 1.15);

    BodyCompletionPose hmd_pose = input.hmd.pose;
    SetRole(result, BodyRole::Hmd, hmd_pose, 1.0f,
        kBodySourceMeasured | kBodySourceHmd,
        BodyCompletionMode::Measured,
        input.hmd.age_ms);
    ...
    SetRole(result, BodyRole::Waist, waist, 0.48f,
        kBodySourceHmd | kBodySourcePredicted,
        BodyCompletionMode::HmdRoot);
    SetRole(result, BodyRole::Chest, chest, 0.58f,
        kBodySourceHmd | kBodySourcePredicted,
        BodyCompletionMode::HmdRoot);
    ...
    BodyCompletionPose right_shoulder =
        OffsetPose(chest, right, up, forward, cal.shoulder_width_m * 0.5, 0.08, 0.0, body_rot);
    SetRole(result, BodyRole::LeftShoulder, left_shoulder, 0.42f,
        kBodySourceHmd | kBodySourcePredicted,
        BodyCompletionMode::HmdRoot);
    SetRole(result, BodyRole::RightShoulder, right_shoulder, 0.42f,
        kBodySourceHmd | kBodySourcePredicted,
        BodyCompletionMode::HmdRoot);

    auto solveArm = [&](BodyRole elbowRole,
                        const BodyCompletionPose& shoulder,
                        const BodyCompletionSensorPose& controller,
                        double side) {
        if (controller.valid) {
            ...
            const float confidence = DegradeByReach(
                shoulder, hand, cal.upper_arm_m + cal.lower_arm_m + 0.05, 0.70f);
            SetRole(result, elbowRole, elbow, confidence,
                kBodySourceHmd | kBodySourceController | kBodySourcePredicted,
                confidence >= 0.25f ? BodyCompletionMode::ControllerIk : BodyCompletionMode::LowConfidence,
                controller.age_ms);
            return;
        }
        BodyCompletionPose elbow =
            OffsetPose(shoulder, right, up, forward,
                side * cal.shoulder_width_m * 0.10,
                -cal.upper_arm_m * 0.65,
                0.03,
                body_rot);
        SetRole(result, elbowRole, elbow, 0.18f,
            kBodySourceHmd | kBodySourcePredicted,
    ...
    BodyCompletionPose left_foot =
        OffsetPose(waist, right, up, forward,
            -cal.stance_width_m * 0.5,
            cal.floor_y_m - waist.position[1] + 0.04,
            0.02,
            body_rot);
    BodyCompletionPose right_foot =
        OffsetPose(waist, right, up, forward,
            cal.stance_width_m * 0.5,
            cal.floor_y_m - waist.position[1] + 0.04,
            0.02,
    ...
    SetRole(result, BodyRole::LeftKnee, left_knee, leftHeld ? 0.34f : 0.22f,
        kBodySourceHmd | kBodySourceFloor | kBodySourcePredicted,
        BodyCompletionMode::FloorContact);
    ...
    for (uint8_t i = 0; i < kBodyRoleCount; ++i) {
        ...
        SetRole(result, role, measured.pose, 0.98f,
            kBodySourceMeasured, BodyCompletionMode::Measured, measured.age_ms);
    }
    ...
    for (uint8_t i = 0; i < kBodyRoleCount; ++i) {
        ...
        if (!input.enabled_roles[i]
            && i != static_cast<uint8_t>(BodyRole::Hmd)
    ...
    result.global_confidence = confidence_count
        ? static_cast<float>(confidence_sum / confidence_count)
        : 0.0f;
    return result;
}
