...
std::wstring ResolveDriverDllPath()
{
    ...
    if (!GetModuleHandleExW(
            ...
            reinterpret_cast<LPCWSTR>(&ResolveDriverDllPath),
            &hMod)) {
        return {};
    }
...
void SidecarSupervisor::SuperviseLoop()
{
    ...
    while (!stop_requested_.load(std::memory_order_acquire)) {
        ...
        const BOOL ok = CreateProcessW(
            ...
            &si, &pi);
        if (!ok) {
            LOG("[phantom] sidecar: CreateProcessW err=%u; retrying after backoff",
        ...
        while (!exit_history.empty()
               && (now - exit_history.front()) > kCircuitBreakerWindow) {
        ...
        if (exit_history.size() >= kCircuitBreakerAttempts) {
            LOG("[phantom] sidecar: circuit breaker tripped after %zu exits "
                ...
                exit_history.size(),
                (long long)std::chrono::duration_cast<std::chrono::minutes>(
                    kCircuitBreakerWindow).count());
            halted_.store(true, std::memory_order_release);
            break;
        }

        const DWORD secs = kBackoffSeconds[
            backoff_idx < std::size(kBackoffSeconds) - 1
                ? backoff_idx++
                : std::size(kBackoffSeconds) - 1];
        for (DWORD elapsed = 0;
             elapsed < secs && !stop_requested_.load(std::memory_order_acquire);
             ++elapsed) {
            Sleep(1000);
        }
    }
