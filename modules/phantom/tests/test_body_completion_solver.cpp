...
phantom::BodyCompletionPose MakePose(double x, double y, double z, const double q[4])
{
...
TEST(BodyCompletionSolverTest, CrouchLowersRootEstimate)
{
	...
	const auto& standingWaist = standingResult.roles[static_cast<uint8_t>(phantom::BodyRole::Waist)];
	const auto& crouchWaist = crouchResult.roles[static_cast<uint8_t>(phantom::BodyRole::Waist)];
...
TEST(BodyCompletionSolverTest, MeasuredRoleOverridesInference)
{
	...
	const double identity[4] = {1.0, 0.0, 0.0, 0.0};
	input.hmd = Sensor(MakePose(0.0, 1.70, 0.0, identity));
	input.real_roles[static_cast<uint8_t>(phantom::BodyRole::LeftFoot)] = Sensor(MakePose(1.0, 0.22, 3.0, identity));
	Enable(input, phantom::BodyRole::LeftFoot);
