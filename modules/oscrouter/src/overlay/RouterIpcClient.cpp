...
openvr_pair::overlay::IpcClientConnectOptions Options()
{
	...
	opts.pipeUnavailable = [](DWORD, const std::string&) {
		return "OSC Router driver unavailable. Neither SteamVR nor the desktop backend is hosting it, "
	...
	opts.pipeModeFailed = [](DWORD error, const std::string& details) {
		return "Could not set OSC Router pipe mode. Error " + std::to_string(error) + ": " + details;
	};
...
{
	openvr_pair::overlay::IpcClientBase::Connect(OPENVR_PAIRDRIVER_OSCROUTER_PIPE_NAME, Options());
}
