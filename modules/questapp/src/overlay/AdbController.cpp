...
{
	const std::wstring toolsDir = openvr_pair::common::WkOpenVrSubdirectoryPath(L"questapp\\platform-tools", false);
...
std::string RedactAdbText(std::string text)
{
	...
	text = std::regex_replace(text, std::regex(R"(([^\s]+)\s+(device|unauthorized|offline)(\s|$))"), "<device> $2$3");
	text = std::regex_replace(text, std::regex(R"(\b\d{1,3}(?:\.\d{1,3}){3}(?::\d+)?\b)"), "<ip>");
	text = std::regex_replace(text, std::regex(R"(\b\d{6}\b)"), "<code>");
	return text;
}
...
bool LooksLikeConnectionRefused(const std::string& out, const std::string& err)
{
	...
	return text.find("refused") != std::string::npos || text.find("actively refused") != std::string::npos ||
	       text.find("cannot connect") != std::string::npos || text.find("failed to connect") != std::string::npos;
}
...
AdbController::AdbController() : m_adbPath(ResolveAdbPath())
{
	if (m_adbPath.empty()) {
...
AdbController::AdbController(std::string adbPath) : m_adbPath(std::move(adbPath)) {}
...
std::string AdbController::QuoteArg(const std::string& arg)
{
	...
	bool needsQuote = arg.empty() || arg.find(' ') != std::string::npos || arg.find('\t') != std::string::npos ||
...
AdbController::Result AdbController::Run(const std::vector<std::string>& args, std::chrono::milliseconds timeout)
{
	Result result;
	...
	if (!CreatePipe(&hStdoutRead, &hStdoutWrite, &sa, 0) || !CreatePipe(&hStderrRead, &hStderrWrite, &sa, 0)) {
		...
		openvr_pair::common::DiagnosticLog("adb", "run_pipe_failed command='%s' winerr=%lu", commandForLog.c_str(),
		                                   pipeErr);
	...
	const BOOL ok = CreateProcessW(exePath.c_str(), cmdLine.data(), nullptr, nullptr,
	...
	if (!ok) {
		...
		openvr_pair::common::DiagnosticLog("adb", "run_spawn_failed command='%s' winerr=%lu", commandForLog.c_str(),
		                                   err);
	...
	openvr_pair::common::DiagnosticLog("adb", "run_spawned command='%s' pid=%lu", commandForLog.c_str(),
	                                   pi.dwProcessId);
	...
	if (waitResult == WAIT_TIMEOUT) {
		...
		openvr_pair::common::DiagnosticLog("adb", "run_timeout command='%s' timeout_ms=%lld", commandForLog.c_str(),
		                                   static_cast<long long>(timeout.count()));
	}
	...
	openvr_pair::common::DiagnosticLog("adb", "run_exit command='%s' exit=%d timed_out=%d stdout='%s' stderr='%s'",
	                                   commandForLog.c_str(), result.exitCode, result.timedOut ? 1 : 0,
	                                   TruncateForLog(result.out).c_str(), TruncateForLog(result.err).c_str());
	return result;
}

AdbController::Result AdbController::Shell(const std::string& cmd, std::chrono::milliseconds timeout)
{
...
bool AdbController::Connect(const std::string& endpoint, std::chrono::milliseconds timeout)
{
	...
	if (r.timedOut) {
		...
		openvr_pair::common::DiagnosticLog(
		    "adb", "connect_result endpoint='%s' ok=0 timed_out=1 refused=%d exit=%d stdout='%s' stderr='%s'",
		    RedactAdbText(endpoint).c_str(), m_lastConnectWasRefused ? 1 : 0, r.exitCode, TruncateForLog(r.out).c_str(),
	...
	const bool ok = (r.exitCode == 0) && (r.out.find("connected to") != std::string::npos);
	if (ok) {
		m_targetSerial = endpoint;
		openvr_pair::common::DiagnosticLog("adb", "target_serial_set source=connect endpoint='%s'", endpoint.c_str());
	}
	openvr_pair::common::DiagnosticLog(
	    "adb", "connect_result endpoint='%s' ok=%d timed_out=0 refused=%d exit=%d stdout='%s' stderr='%s'",
	    RedactAdbText(endpoint).c_str(), ok ? 1 : 0, m_lastConnectWasRefused ? 1 : 0, r.exitCode,
	    TruncateForLog(r.out).c_str(), TruncateForLog(r.err).c_str());
	fprintf(stderr, "[adb] connect endpoint='%s' ok=%d exit=%d out='%s'\n", endpoint.c_str(), ok ? 1 : 0, r.exitCode,
	        TrimAscii(r.out).c_str());
	return ok;
}
...
bool AdbController::Disconnect(const std::string& endpoint)
{
	...
	if (ok && (endpoint.empty() || endpoint == m_targetSerial)) {
		...
		openvr_pair::common::DiagnosticLog("adb", "target_serial_cleared source=disconnect endpoint='%s'",
		                                   endpoint.c_str());
	}
...
bool AdbController::DisableWirelessAdb(const std::string& endpoint)
{
	...
	if (ok && !endpoint.empty() && endpoint == m_targetSerial) {
		m_targetSerial.clear();
		openvr_pair::common::DiagnosticLog("adb", "target_serial_cleared source=usb endpoint='%s'", endpoint.c_str());
	}
	fprintf(stderr, "[adb] usb endpoint='%s' ok=%d exit=%d out='%s' err='%s'\n", endpoint.c_str(), ok ? 1 : 0,
	        r.exitCode, TrimAscii(r.out).c_str(), TrimAscii(r.err).c_str());
	return ok;
}
...
bool AdbController::EnableWirelessAdb(const std::string& serial, int port)
{
	...
	const bool ok = (r.exitCode == 0);
	fprintf(stderr, "[adb] tcpip serial='%s' port=%d ok=%d exit=%d out='%s' err='%s'\n", RedactAdbText(serial).c_str(),
	        port, ok ? 1 : 0, r.exitCode, TrimAscii(r.out).c_str(), TrimAscii(r.err).c_str());
	return ok;
}
...
bool AdbController::OpenInteractiveShell(const std::string& serial) const
{
	...
	HINSTANCE launched = ShellExecuteW(nullptr, L"open", L"cmd.exe", args.c_str(),
...
bool AdbController::OpenToolsTerminal() const
{
	...
	HINSTANCE launched = ShellExecuteW(nullptr, L"open", L"cmd.exe", L"/K",
	                                   directory.empty() ? nullptr : directory.c_str(), SW_SHOWNORMAL);
	const bool ok = reinterpret_cast<intptr_t>(launched) > 32;
