...
{
	switch (family) {
		case inputhealth::PathFamily::TriggerValue:
			++c.trigger_value;
			break;
		case inputhealth::PathFamily::ThumbstickAxis:
			++c.thumbstick_axis;
			break;
		case inputhealth::PathFamily::TrackpadAxis:
			++c.trackpad_axis;
			break;
		case inputhealth::PathFamily::ForceSensor:
			++c.force_sensor;
			break;
		case inputhealth::PathFamily::GripValue:
			++c.grip_value;
			break;
		case inputhealth::PathFamily::FingerCapsense:
			++c.finger_capsense;
			break;
		case inputhealth::PathFamily::ControllerButton:
			++c.controller_button;
			break;
		case inputhealth::PathFamily::DiagnosticsOnly:
			++c.diagnostics_only;
			break;
		case inputhealth::PathFamily::Unsupported:
			++c.unsupported;
			break;
	}
}
...
std::string BuildInputHealthHealthSummaryJson(const InputHealthHealthSummarySnapshot& snapshot)
{
	...
	learning["profile_sync_skipped_sample_churn"] = CountValue(snapshot.learning.profile_sync_skipped_sample_churn);
	learning["profile_sync_skipped_periodic_throttle"] =
	    CountValue(snapshot.learning.profile_sync_skipped_periodic_throttle);
	learning["last_profile_save_reason"] = picojson::value(snapshot.learning.last_profile_save_reason);
	learning["top_drift_paths"] = picojson::value(TopPaths(snapshot.learning.diagnostic_path_counts));
