...
TEST(FingerSmoothingConfigPacking, RoundTripsHeaderAndPerFingerValues)
{
    ...
    const auto unpacked = pairdriver::UnpackFingerSmoothing(
        pairdriver::PackFingerHeader(cfg),
        pairdriver::PackFingerLow(cfg));
...
TEST(FingerSmoothingConfigPacking, ComputesReseedBitsOnlyWhenFingerBecomesSmoothed)
{
    ...
    EXPECT_EQ(pairdriver::ComputeFingerSmoothingReseedBits(prev, next),
        protocol::kAllFingersMask);
...
TEST(SmoothFingerFrame, GlobalZeroPassesThroughAfterSeed)
{
    ...
    const auto seed = skeletal::math::SmoothFingerFrame(
        state, first, kFingerBoneCount, 0, protocol::kAllFingersMask, alpha, output);
    EXPECT_TRUE(seed.seeded);
    EXPECT_TRUE(state.initialized);

    const auto result = skeletal::math::SmoothFingerFrame(
        state, second, kFingerBoneCount, 0, protocol::kAllFingersMask, alpha, output);
...
TEST(SmoothFingerFrame, GlobalStrengthSmoothsEnabledFingerOnly)
{
    ...
    FingerFrameState state{};
    ASSERT_TRUE(skeletal::math::SmoothFingerFrame(
        state, first, kFingerBoneCount, 0, protocol::kAllFingersMask, alpha, output).seeded);
...
TEST(SmoothFingerFrame, MaskedFingerPassesThrough)
{
    ...
    const auto result = skeletal::math::SmoothFingerFrame(
        state, second, kFingerBoneCount, 0, maskWithoutLeftIndex, alpha, output);
    EXPECT_TRUE(result.appliedSmoothing);
...
TEST(SmoothFingerFrame, EmptyMaskPassesAllBonesThrough)
{
    ...
    const auto result = skeletal::math::SmoothFingerFrame(
        state, second, kFingerBoneCount, 0, 0, alpha, output);
    EXPECT_FALSE(result.appliedSmoothing);
    for (uint32_t i = 0; i < kFingerBoneCount; ++i) {
        ExpectBoneNear(output[i], second[i]);
        ExpectBoneNear(state.previous[i], second[i]);
...
TEST(SmoothFingerFrame, ReseedPassesRawForOneFrameThenSmooths)
{
    ...
    const auto reseed = skeletal::math::SmoothFingerFrame(
        state, second, kFingerBoneCount, 0, leftIndexMask, alpha, output);
    EXPECT_TRUE(reseed.reseeded);
    EXPECT_FALSE(reseed.appliedSmoothing);
    ExpectBoneNear(output[6], second[6]);
    EXPECT_FALSE(state.reseed_pending[1]);

    const auto smoothed = skeletal::math::SmoothFingerFrame(
        state, third, kFingerBoneCount, 0, leftIndexMask, alpha, output);
    EXPECT_TRUE(smoothed.appliedSmoothing);
    EXPECT_NEAR(output[6].position.v[0],
        (second[6].position.v[0] + third[6].position.v[0]) * 0.5f,
        kEpsilon);
}
